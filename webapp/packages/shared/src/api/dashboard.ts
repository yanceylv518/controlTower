...
export interface MetricItem { instance_id: string; instance_name: string; bucket_time: string; dimension_type: string; dimension_key: string; display_key: string; request_count: number; success_count: number; error_count: number; success_rate: number | null; error_rate: number | null; tpm: number; prompt_tokens: number; completion_tokens: number; quota: number; avg_use_time: number | null; p95_use_time: number | null; p50_use_time?: number | null; p99_use_time?: number | null; stream_rate: number | null; cache_token_rate: number | null; big_input_count: number | null; big_input_cache_hits: number | null; cache_hit_rate: number | null; ttft_count: number | null; ttft_avg_ms: number | null; ttft_p95_ms: number | null }
export interface AlertItem { id: string; instance_id: string; instance_name: string; display_key: string; dimension_type: string; dimension_key: string; rule_key: string; severity: string; status: string; title: string; summary: string; seen_at: string; first_seen_at: string; last_seen_at: string; resolved_at?: string; silence_until?: string }
...
export interface ChannelSnapshot { id: string; instance_id: string; instance_name: string; channel_id: number; channel_name: string; status: string; weight: number; models_text: string; group_name: string | null; priority: number | null; captured_at: string }
...
export const dashboardApi = (client: ApiClient) => ({
  ...
  metricHistory: (params: { window: string; dimension_type: string; dimension_key: string; hours: number; aggregate?: boolean }) => client.request<ListResponse<MetricItem>>(`/api/dashboard/metric-history${query(params)}`),
