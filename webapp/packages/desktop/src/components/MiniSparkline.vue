...
const visibleColor = computed(() => { themeSignature.value; return chartColor(props.color); });
...
<template>
  <svg class="mini-spark" viewBox="0 0 100 30" preserveAspectRatio="none" aria-hidden="true">
    <template v-if="bars">
      <rect v-for="(value, index) in clean" :key="index" :x="index * (100 / clean.length) + 1" :y="28 - (value / max) * 24" :width="Math.max(1, 100 / clean.length - 2)" :height="Math.max(1, (value / max) * 24)" rx="1" :fill="visibleColor" opacity=".8" />
    </template>
    <polyline v-else :points="points" fill="none" :stroke="visibleColor" stroke-width="2" vector-effect="non-scaling-stroke" />
  </svg>
</template>
