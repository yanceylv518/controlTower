...
watch(() => props.series, () => void render(), { deep: true, immediate: true })
watch(() => props.percent, () => void render())
watch(chartEl, element => { observer?.disconnect(); if (element) { observer = new ResizeObserver(() => chart?.resize()); observer.observe(element) } })
onBeforeUnmount(() => { observer?.disconnect(); chart?.dispose() })
</script>

<template><section class="trend-chart"><h3>{{ title }}</h3><div v-if="hasData" ref="chartEl" class="trend-chart-canvas"></div><el-empty v-else :image-size="52" description="暂无趋势数据" /></section></template>
