  ...
  <AppShell :title="title">
    ...
    <el-dialog v-model="detailVisible" width="min(1180px, calc(100vw - 48px))" top="7vh" class="bill-preview-dialog" destroy-on-close>
      ...
      <div v-loading="detailLoading" class="preview-body">
        ...
        <el-alert type="info" :closable="false" :title="detail.job.pricing_source === 'newapi' ? '计费来源：NewAPI 原始扣费；历史单价与规则仅供核对，不改变金额。上游渠道折扣另行应用。' : '计费来源：重新计费。'" :description="detail.job.usage_version ? '普通输入/输出与图像、音频分别统计；缓存单列。日志未记录的多媒体用量不估算；用量拆分不改变金额。' : '历史账单未保存多媒体用量拆分；需要明细时请新建账单任务。'" />
