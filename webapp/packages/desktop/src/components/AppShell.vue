...
const visibleNav = computed(() => nav.map(section => ({ ...section, items: section.items.filter(item => canVisit(auth.user, item[0])) })).filter(section => section.items.length));
  ...
  <div class="shell" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
    <aside class="sidebar">
      ...
      <nav>
        ...
        <template v-else>
          <router-link v-if="canVisit(auth.user, '/')" to="/" class="nav-home" :title="sidebarCollapsed ? '总览' : undefined">
