  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      <el-table-column type="expand" width="36"><template #default="s"><div class="expanded">
        <div class="detail-row"><span>渠道信息</span><strong>{{channelInfo(s.row)}}</strong></div><div class="detail-row"><span>Request ID</span><strong>{{s.row.request_id||'—'}}</strong></div><div class="detail-row"><span>缓存 Tokens</span><strong>{{formatNumber(cacheTokens(s.row))}}</strong></div><div class="detail-row"><span>日志详情</span><strong>{{s.row.content_summary||'—'}}</strong></div>
        <div class="detail-row detail-multiline"><span>计费过程</span><div><div v-for="line in billingLines(s.row)" :key="line">{{line}}</div></div></div><div class="detail-row"><span>请求路径</span><strong>{{requestPath(s.row)}}</strong></div><div class="detail-row"><span>流状态</span><strong>{{streamStatus(s.row)}}</strong></div><div class="detail-row"><span>参数覆盖</span><strong>{{overrideText(s.row)||'无'}}</strong></div><div class="detail-row"><span>请求转换</span><strong>{{conversion(s.row)}}</strong></div><div class="detail-row"><span>计费模式</span><strong>{{billingMode(s.row)}}</strong></div>
      </div></template></el-table-column>
      <el-table-column label="时间" width="172"><template #default="s">{{new Date(s.row.created_at).toLocaleString()}}</template></el-table-column><el-table-column label="渠道" width="70"><template #default="s"><span class="pill pill-channel">{{s.row.channel_id}}</span></template></el-table-column><el-table-column label="用户" min-width="120"><template #default="s"><span class="user-cell"><i>{{initial(s.row.username)}}</i>{{s.row.username||`用户 ${s.row.user_id}`}}</span></template></el-table-column><el-table-column label="令牌" min-width="130" show-overflow-tooltip><template #default="s"><span class="pill pill-token">{{s.row.token_name||'—'}}</span></template></el-table-column><el-table-column label="分组" width="82"><template #default="s"><span class="pill pill-group">{{s.row.group||'—'}}</span></template></el-table-column><el-table-column label="类型" width="74"><template #default="s"><el-tag :type="typeMeta(s.row.type)[1]" round size="small">{{typeMeta(s.row.type)[0]}}</el-tag></template></el-table-column><el-table-column label="模型" min-width="155" show-overflow-tooltip><template #default="s"><span class="pill pill-model">{{s.row.model_name||'—'}}</span></template></el-table-column>
      <el-table-column label="用时/首字" width="132"><template #default="s"><span class="metric-pill">{{s.row.use_time}} s</span><span v-if="firstResponse(s.row)!==undefined" class="metric-pill warm">{{firstResponse(s.row)?.toFixed(1)}} s</span><span class="stream-pill">{{s.row.is_stream?'流':'非流'}}</span></template></el-table-column><el-table-column label="输入" width="105"><template #default="s"><div>{{formatNumber(s.row.prompt_tokens)}}</div><small v-if="cacheTokens(s.row)">缓存读 {{formatNumber(cacheTokens(s.row))}}</small></template></el-table-column><el-table-column label="输出" width="84"><template #default="s">{{formatNumber(s.row.completion_tokens)}}</template></el-table-column><el-table-column label="花费" width="105"><template #default="s">{{money(s.row.quota)}}</template></el-table-column><el-table-column label="IP" min-width="120" show-overflow-tooltip><template #default="s">{{s.row.ip||'—'}}</template></el-table-column><el-table-column label="重试" width="74"><template #default="s">{{textValue(s.row,'retry_count')||'—'}}</template></el-table-column><el-table-column label="详情" min-width="205" show-overflow-tooltip><template #default="s">{{s.row.content_summary||`渠道：${s.row.channel_id}`}}</template></el-table-column>
    </el-table>
  </div>
