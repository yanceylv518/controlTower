...
const reset = () => { username.value = ''; tokenName.value = ''; modelName.value = ''; requestID.value = ''; channelID.value = ''; logType.value = 0; timeRange.value = defaultTimeRange(); search() }
// Click-to-copy for queryable values. navigator.clipboard needs a secure
// context; plain-http deployments fall back to the textarea trick.
...
function billingSummary(row: ReadonlyLog) {
  ...
  if (isPerCallBilling(modelPrice)) {
    ...
    parts.push(`输入 ${billingPrice(inputPrice)}/1M`)
    if (completionRatio !== undefined) parts.push(`输出 ${billingPrice(inputPrice * completionRatio)}/1M`)
    if (cacheReadTokens(row) > 0 && cacheRatio !== undefined && cacheRatio !== 1) parts.push(`缓存读 ${billingPrice(inputPrice * cacheRatio)}/1M`)
    ...
    if (cacheWriteTokens(row) > 0 && cacheWriteRatio !== undefined && cacheWriteRatio !== 1) parts.push(`缓存写 ${billingPrice(inputPrice * cacheWriteRatio)}/1M`)
    if (cacheWrite1hTokens(row) > 0 && cacheWrite1hRatio !== undefined && cacheWrite1hRatio !== 0) parts.push(`缓存写1h ${billingPrice(inputPrice * cacheWrite1hRatio)}/1M`)
  }
...
<template><AppShell title="使用日志"><div class="logs-page">
  ...
  <div class="filter-bar">
    <div class="filter-fields">
      <el-date-picker v-model="timeRange" type="datetimerange" range-separator="~" start-placeholder="开始时间" end-placeholder="结束时间" :shortcuts="timeRangeShortcuts" unlink-panels :clearable="false" class="f-time"/>
      <el-select v-model="logType" placeholder="全部类型" class="f-select"><el-option label="全部类型" :value="0"/><el-option label="消费" :value="2"/><el-option label="错误" :value="5"/><el-option label="充值" :value="1"/><el-option label="管理" :value="3"/><el-option label="系统" :value="4"/><el-option label="退款" :value="6"/><el-option label="登录" :value="7"/></el-select>
      <el-input v-model="modelName" clearable prefix-icon="Search" placeholder="模型名称" @keyup.enter="search" class="f-input"/>
      <el-input v-model="tokenName" clearable prefix-icon="Search" placeholder="令牌名称" @keyup.enter="search" class="f-input"/>
      <el-input v-model="username" clearable prefix-icon="Search" placeholder="用户名称" @keyup.enter="search" class="f-input"/>
      <el-input v-model="channelID" clearable prefix-icon="Search" placeholder="渠道 ID" @keyup.enter="search" class="f-narrow"/>
      <el-input v-model="requestID" clearable prefix-icon="Search" placeholder="Request ID" @keyup.enter="search" class="f-input"/>
  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      <el-table-column type="expand" width="36"><template #default="s"><div class="expanded">
        <div class="detail-row"><span>Request ID</span><strong :class="{copyable:!!s.row.request_id}" :title="s.row.request_id?'点击复制':undefined" @click="copyText(s.row.request_id)">{{s.row.request_id||'—'}}</strong></div>
      ...
      <el-table-column label="时间" min-width="180"><template #default="s"><div class="time-cell" :title="timeFull(s.row.created_at)"><span class="time-text">{{timeText(s.row.created_at)}}</span><el-tag :type="typeMeta(s.row.type)[1]" round size="small" class="time-tag">{{typeMeta(s.row.type)[0]}}</el-tag></div></template></el-table-column>
      <el-table-column label="渠道" min-width="150"><template #default="s"><span v-if="s.row.channel_id" class="channel-id copyable" title="点击复制" @click="copyText(s.row.channel_id)">#{{s.row.channel_id}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="用户" min-width="145" show-overflow-tooltip><template #default="s"><span class="user-cell copyable" title="点击复制" @click="copyText(s.row.username||s.row.user_id)"><i>{{initial(s.row.username)}}</i><span class="cell-ellipsis">{{s.row.username||`用户 ${s.row.user_id}`}}</span></span></template></el-table-column>
      <el-table-column label="令牌" min-width="190" show-overflow-tooltip><template #default="s"><span v-if="s.row.token_name" class="pill pill-token copyable" title="点击复制" @click="copyText(s.row.token_name)">{{s.row.token_name}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="模型" min-width="190" show-overflow-tooltip><template #default="s"><span v-if="s.row.model_name" class="pill pill-model copyable" title="点击复制" @click="copyText(s.row.model_name)">{{s.row.model_name}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="流" min-width="110"><template #default="s"><div v-if="isConsume(s.row)" class="stream-cell"><span class="stream-label" :class="{ unknown: streamFlag(s.row)===undefined }">{{streamLabel(s.row)}}</span><small v-if="outputRate(s.row)">{{formatNumber(outputRate(s.row))}} t/s</small></div><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="Tokens" min-width="155" align="right"><template #default="s"><template v-if="s.row.prompt_tokens||s.row.completion_tokens"><div class="tokens-line"><span>{{formatNumber(s.row.prompt_tokens)}}</span><b>/</b><span>{{formatNumber(s.row.completion_tokens)}}</span></div><small v-if="cacheReadTokens(s.row)||cacheWriteTokens(s.row)"><template v-if="cacheReadTokens(s.row)">缓存↓ {{formatNumber(cacheReadTokens(s.row))}}</template><template v-if="cacheWriteTokens(s.row)"> ↑ {{formatNumber(cacheWriteTokens(s.row))}}</template></small></template><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="费用" min-width="140" align="right"><template #default="s"><span v-if="s.row.quota||isConsume(s.row)" class="cost-pill">{{money(s.row.quota)}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="耗时" min-width="150"><template #default="s"><div v-if="isConsume(s.row)" class="duration-cell"><span v-if="firstResponse(s.row)!==undefined">首字 <b>{{firstResponse(s.row)?.toFixed(1)}}s</b></span><span>耗时 <b>{{s.row.use_time.toFixed(1)}}s</b></span></div><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="详情" min-width="190" show-overflow-tooltip><template #default="s"><span class="billing-summary">{{billingSummary(s.row).replaceAll('；',' · ')}}</span></template></el-table-column>
    </el-table>
  </div>
  <div class="pager">
    <div class="pager-summary">
      ...
      <span class="pager-range">第 {{rangeStart}}-{{rangeEnd}} 条<span v-if="countState.loading.value">，总数统计中…</span><span v-else-if="countState.error.value">，总数暂不可用</span><span v-else>，共 {{formatNumber(effectiveTotal)}} 条</span></span>
    </div>
...
<style scoped>.logs-page{display:flex;flex-direction:column;gap:10px;height:calc(100vh - 88px)}.logs-page>.el-alert{flex:none}.summary-chip{padding:5px 11px;border-radius:8px;font-size:13px;white-space:nowrap}.summary-chip.quota{background:#e7f0ff;color:#2266bd}.summary-chip.rpm{background:#ffe6ef;color:#b51f56}.summary-chip.tpm{background:#f2f3f5;color:#303133}.filter-bar{flex:none;padding:10px 14px;background:#fff;border:1px solid var(--el-border-color-light);border-radius:8px}.filter-fields{display:flex;flex-wrap:wrap;align-items:center;gap:8px}.filter-fields .f-time{width:400px;flex:none}.filter-fields .f-select{width:118px;flex:none}.filter-fields .f-input{width:172px;flex:none}.filter-fields .f-narrow{width:120px;flex:none}.filter-buttons{display:flex;gap:8px;white-space:nowrap;margin-left:auto}.table-panel{flex:1;min-height:0;background:#fff;border:1px solid var(--el-border-color-light);border-radius:8px;overflow:hidden}.table-panel :deep(.el-table__header th){height:42px;background:#fafbfc;color:#4e5969;font-weight:600}.table-panel :deep(.el-table__row td){padding:9px 0}.table-panel :deep(.el-table__row:hover td){background:#f7f9fc}.pager{flex:none;display:flex;justify-content:space-between;align-items:center;gap:18px;min-height:48px;padding:8px 12px;margin-top:-10px;color:#606266;font-size:13px;background:#fff;border:1px solid var(--el-border-color-light);border-top:0;border-radius:0 0 8px 8px;box-sizing:border-box}.pager-summary{display:flex;align-items:center;gap:8px;min-width:0;flex-wrap:wrap}.pager-range{white-space:nowrap;margin-left:4px}.pager-controls{display:flex;align-items:center;gap:14px;min-width:0}.pager :deep(.el-pagination){flex-wrap:nowrap}.expanded{margin:0 12px 0 18px;padding:10px 18px;background:#f5f7fa}.detail-row{display:grid;grid-template-columns:100px minmax(0,1fr);gap:16px;min-height:28px;align-items:start;color:#303133}.detail-row>span{color:#73767a;text-align:right}.detail-row strong{font-weight:400;word-break:break-word}.detail-multiline{white-space:pre-line;line-height:1.55}.billing-summary{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.45;color:#4e5969}.pill{display:inline-block;max-width:100%;padding:3px 9px;border-radius:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pill-token{background:#edf0f2;color:#303133}.pill-model{background:#f1e8fb;color:#7134a3}.channel-id{color:#3b82f6;font-variant-numeric:tabular-nums}.user-cell{display:flex;align-items:center;gap:7px;min-width:0}.user-cell i{display:inline-flex;flex:none;align-items:center;justify-content:center;width:24px;height:24px;border-radius:50%;background:#43c6b9;color:#fff;font-style:normal}.cell-ellipsis{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stream-cell,.duration-cell{display:flex;flex-direction:column;align-items:flex-start;gap:2px;white-space:nowrap}.duration-cell span{font-size:12px;color:#606266}.duration-cell b{color:#169b62;font-weight:500}.duration-cell span:first-child b{color:#d88913}.cost-pill{display:inline-block;padding:3px 8px;border:1px solid #e5e7eb;border-radius:12px;background:#fafafa;color:#202124;font-weight:600;font-variant-numeric:tabular-nums}.stream-label{display:inline-block;padding:1px 6px;border-radius:9px;background:#dce9ff;color:#2468c9;font-size:12px;line-height:18px;white-space:nowrap}.stream-label.unknown{background:#f0f2f5;color:#909399}.table-panel small{display:block;color:#909399;white-space:nowrap}.copyable{cursor:pointer}.copyable:hover{text-decoration:underline dotted;text-underline-offset:3px}.time-cell{display:flex;flex-direction:column;gap:3px;align-items:flex-start}.time-text{font-variant-numeric:tabular-nums;font-size:12px;color:#303133;white-space:nowrap}.time-tag{margin-left:-2px}.tokens-line{display:flex;justify-content:flex-end;gap:6px;font-variant-numeric:tabular-nums}.tokens-line b{color:#c0c4cc;font-weight:400}.dim{color:#c0c4cc}@media(max-width:1200px){.pager{align-items:flex-start;flex-direction:column;height:auto}.pager-controls{width:100%;justify-content:flex-end}}</style>
