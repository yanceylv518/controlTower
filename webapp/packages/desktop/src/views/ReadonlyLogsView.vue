...
<template><AppShell title="使用日志"><div class="logs-page">
  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      ...
      <el-table-column label="时间" min-width="112"><template #default="s"><div class="time-cell" :title="timeFull(s.row.created_at)"><span class="time-text">{{timeText(s.row.created_at)}}</span><el-tag :type="typeMeta(s.row.type)[1]" round size="small" class="time-tag">{{typeMeta(s.row.type)[0]}}</el-tag></div></template></el-table-column>
      <el-table-column label="渠道" min-width="58"><template #default="s"><span v-if="s.row.channel_id" class="channel-id copyable" title="点击复制" @click="copyText(s.row.channel_id)">#{{s.row.channel_id}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="用户" min-width="124" show-overflow-tooltip><template #default="s"><span class="user-cell copyable" title="点击复制" @click="copyText(s.row.username||s.row.user_id)"><i>{{initial(s.row.username)}}</i><span class="cell-ellipsis">{{s.row.username||`用户 ${s.row.user_id}`}}</span></span></template></el-table-column>
      <el-table-column label="令牌" min-width="124" show-overflow-tooltip><template #default="s"><span v-if="s.row.token_name" class="pill pill-token copyable" title="点击复制" @click="copyText(s.row.token_name)">{{s.row.token_name}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="模型" min-width="160" show-overflow-tooltip><template #default="s"><span v-if="s.row.model_name" class="pill pill-model copyable" title="点击复制" @click="copyText(s.row.model_name)">{{s.row.model_name}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="流" min-width="70"><template #default="s"><div v-if="isConsume(s.row)" class="stream-cell"><span class="stream-label" :class="{ unknown: streamFlag(s.row)===undefined }">{{streamLabel(s.row)}}</span><small v-if="outputRate(s.row)">{{formatNumber(outputRate(s.row))}} t/s</small></div><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="Tokens" min-width="124" align="right"><template #default="s"><template v-if="s.row.prompt_tokens||s.row.completion_tokens"><div class="tokens-line"><span>{{formatNumber(s.row.prompt_tokens)}}</span><b>/</b><span>{{formatNumber(s.row.completion_tokens)}}</span></div><small v-if="cacheReadTokens(s.row)||cacheWriteTokens(s.row)"><template v-if="cacheReadTokens(s.row)">缓存↓ {{formatNumber(cacheReadTokens(s.row))}}</template><template v-if="cacheWriteTokens(s.row)"> ↑ {{formatNumber(cacheWriteTokens(s.row))}}</template></small></template><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="费用" min-width="88" align="right"><template #default="s"><span v-if="s.row.quota||isConsume(s.row)" class="cost-pill">{{money(s.row.quota)}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="耗时" min-width="100"><template #default="s"><div v-if="isConsume(s.row)" class="duration-cell"><span v-if="firstResponse(s.row)!==undefined">首字 <b>{{firstResponse(s.row)?.toFixed(1)}}s</b></span><span>耗时 <b>{{s.row.use_time.toFixed(1)}}s</b></span></div><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="详情" min-width="148" show-overflow-tooltip><template #default="s"><span class="billing-summary">{{billingSummary(s.row).replaceAll('；',' · ')}}</span></template></el-table-column>
    </el-table>
  </div>
  <div class="pager">
