...
<template><AppShell title="使用日志"><div class="logs-page">
  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      ...
      <el-table-column label="时间" width="150"><template #default="s">{{new Date(s.row.created_at).toLocaleString()}}</template></el-table-column><el-table-column label="类型" width="70"><template #default="s"><el-tag :type="typeMeta(s.row.type)[1]" round size="small">{{typeMeta(s.row.type)[0]}}</el-tag></template></el-table-column><el-table-column label="用户" width="140" show-overflow-tooltip><template #default="s"><span class="user-cell copyable" title="点击复制" @click="copyText(s.row.username||s.row.user_id)"><i>{{initial(s.row.username)}}</i>{{s.row.username||`用户 ${s.row.user_id}`}}</span></template></el-table-column><el-table-column label="令牌" width="140" show-overflow-tooltip><template #default="s"><span class="pill pill-token" :class="{copyable:!!s.row.token_name}" :title="s.row.token_name?'点击复制':undefined" @click="copyText(s.row.token_name)">{{s.row.token_name||'—'}}</span></template></el-table-column><el-table-column label="模型" min-width="160" show-overflow-tooltip><template #default="s"><span class="pill pill-model" :class="{copyable:!!s.row.model_name}" :title="s.row.model_name?'点击复制':undefined" @click="copyText(s.row.model_name)">{{s.row.model_name||'—'}}</span></template></el-table-column><el-table-column label="渠道 ID" width="70"><template #default="s"><span v-if="s.row.channel_id" class="copyable" title="点击复制" @click="copyText(s.row.channel_id)">{{s.row.channel_id}}</span><span v-else>—</span></template></el-table-column>
      <el-table-column label="用时/首字" width="134"><template #default="s"><div class="timing-cell"><span class="metric-pill">{{s.row.use_time}} s</span><span v-if="firstResponse(s.row)!==undefined" class="metric-pill warm">{{firstResponse(s.row)?.toFixed(1)}} s</span><span class="stream-label" :class="{ unknown: streamFlag(s.row)===undefined }">{{streamLabel(s.row)}}</span></div></template></el-table-column><el-table-column label="输入" width="110" align="right"><template #default="s"><div>{{formatNumber(s.row.prompt_tokens)}}</div><small v-if="cacheReadTokens(s.row)">缓存读 {{formatNumber(cacheReadTokens(s.row))}}</small><small v-if="cacheWriteTokens(s.row)">缓存写 {{formatNumber(cacheWriteTokens(s.row))}}</small></template></el-table-column><el-table-column label="输出" width="80" align="right"><template #default="s">{{formatNumber(s.row.completion_tokens)}}</template></el-table-column><el-table-column label="花费" width="98" align="right"><template #default="s">{{money(s.row.quota)}}</template></el-table-column><el-table-column label="详情" min-width="260" align="right" header-align="right"><template #default="s"><div class="billing-summary" v-for="part in billingSummary(s.row).split('；')" :key="part">{{part}}</div></template></el-table-column>
    </el-table>
  </div>
  <div class="pager">
