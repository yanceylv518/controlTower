...
<template><AppShell title="使用日志"><div class="logs-page">
  ...
  <div class="filter-bar">
    <div class="filter-fields">
      ...
      <el-input v-model="modelName" clearable prefix-icon="Search" placeholder="模型名称" class="f-input"/>
      <el-input v-model="tokenName" clearable prefix-icon="Search" placeholder="令牌名称" class="f-input"/>
      <el-input v-model="username" clearable prefix-icon="Search" placeholder="用户名称" class="f-input"/>
      <el-input v-model="channelID" clearable prefix-icon="Search" placeholder="渠道 ID" class="f-narrow"/>
      <el-input v-model="requestID" clearable prefix-icon="Search" placeholder="Request ID" class="f-input"/>
  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      ...
      <el-table-column label="时间" width="160"><template #default="s">{{new Date(s.row.created_at).toLocaleString()}}</template></el-table-column><el-table-column label="渠道 ID" width="76"><template #default="s">{{s.row.channel_id||'—'}}</template></el-table-column><el-table-column label="用户" min-width="150" show-overflow-tooltip><template #default="s"><span class="user-cell"><i>{{initial(s.row.username)}}</i>{{s.row.username||`用户 ${s.row.user_id}`}}</span></template></el-table-column><el-table-column label="令牌" min-width="140" show-overflow-tooltip><template #default="s"><span class="pill pill-token">{{s.row.token_name||'—'}}</span></template></el-table-column><el-table-column label="类型" width="70"><template #default="s"><el-tag :type="typeMeta(s.row.type)[1]" round size="small">{{typeMeta(s.row.type)[0]}}</el-tag></template></el-table-column><el-table-column label="模型" min-width="170" show-overflow-tooltip><template #default="s"><span class="pill pill-model">{{s.row.model_name||'—'}}</span></template></el-table-column>
