...
<template><AppShell title="使用日志"><div class="logs-page">
  ...
  <div class="filter-bar">
    <div class="filter-fields">
      ...
      <el-select v-model="logType" clearable placeholder="全部类型" class="f-select"><el-option label="消费" :value="2"/><el-option label="错误" :value="5"/><el-option label="充值" :value="1"/><el-option label="管理" :value="3"/><el-option label="系统" :value="4"/><el-option label="退款" :value="6"/><el-option label="登录" :value="7"/></el-select>
  ...
  <div v-loading="state.loading.value" class="table-panel">
    <el-table :data="state.data.value?.items||[]" height="100%" empty-text="暂无日志数据" row-key="id">
      ...
      <el-table-column label="时间" width="118"><template #default="s"><div class="time-cell" :title="timeFull(s.row.created_at)"><span class="time-text">{{timeText(s.row.created_at)}}</span><el-tag :type="typeMeta(s.row.type)[1]" round size="small" class="time-tag">{{typeMeta(s.row.type)[0]}}</el-tag></div></template></el-table-column><el-table-column label="渠道" width="56" align="center"><template #default="s"><span v-if="s.row.channel_id" class="copyable" title="点击复制" @click="copyText(s.row.channel_id)">{{s.row.channel_id}}</span><span v-else class="dim">—</span></template></el-table-column><el-table-column label="用户" width="132" show-overflow-tooltip><template #default="s"><span class="user-cell copyable" title="点击复制" @click="copyText(s.row.username||s.row.user_id)"><i>{{initial(s.row.username)}}</i>{{s.row.username||`用户 ${s.row.user_id}`}}</span></template></el-table-column><el-table-column label="令牌" width="128" show-overflow-tooltip><template #default="s"><span v-if="s.row.token_name" class="pill pill-token copyable" title="点击复制" @click="copyText(s.row.token_name)">{{s.row.token_name}}</span><span v-else class="dim">—</span></template></el-table-column><el-table-column label="模型" width="150" show-overflow-tooltip><template #default="s"><span v-if="s.row.model_name" class="pill pill-model copyable" title="点击复制" @click="copyText(s.row.model_name)">{{s.row.model_name}}</span><span v-else class="dim">—</span></template></el-table-column>
      <el-table-column label="Tokens" width="128" align="right"><template #default="s"><template v-if="s.row.prompt_tokens||s.row.completion_tokens"><div class="tokens-line">{{formatNumber(s.row.prompt_tokens)}} / {{formatNumber(s.row.completion_tokens)}}</div><small v-if="cacheReadTokens(s.row)||cacheWriteTokens(s.row)"><template v-if="cacheReadTokens(s.row)">缓存↓{{formatNumber(cacheReadTokens(s.row))}}</template><template v-if="cacheWriteTokens(s.row)"> ↑{{formatNumber(cacheWriteTokens(s.row))}}</template></small></template><span v-else class="dim">—</span></template></el-table-column><el-table-column label="花费" width="92" align="right"><template #default="s"><span v-if="s.row.quota||isConsume(s.row)">{{money(s.row.quota)}}</span><span v-else class="dim">—</span></template></el-table-column><el-table-column label="用时/首字" width="128"><template #default="s"><div v-if="isConsume(s.row)" class="timing-cell"><span class="metric-pill">{{s.row.use_time}} s</span><span v-if="firstResponse(s.row)!==undefined" class="metric-pill warm">{{firstResponse(s.row)?.toFixed(1)}} s</span><span class="stream-label" :class="{ unknown: streamFlag(s.row)===undefined }">{{streamLabel(s.row)}}</span></div><span v-else class="dim">—</span></template></el-table-column><el-table-column label="详情" min-width="190" align="right" header-align="right"><template #default="s"><div class="billing-summary" v-for="part in billingSummary(s.row).split('；')" :key="part">{{part}}</div></template></el-table-column>
    </el-table>
  </div>
  <div class="pager">
