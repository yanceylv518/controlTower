...
async function load() {
 ...
 loading.value = true
 try { const result = await client.request<{ items: Item[]; month: string }>(`/api/dashboard/log-archives?site_id=${encodeURIComponent(site)}${month.value ? `&month=${encodeURIComponent(month.value)}` : ''}`); if (ticket === sequence && site === filters.site_id) { item.value = result.items[0]; month.value = result.month || month.value || beijingDate().slice(0,7); failure.value = '' } }
...
watch(() => filters.site_id, () => { dialog.value = false; checkDialog.value = false; item.value = undefined; month.value = ''; failure.value = ''; void load() })
function changeMonth() { if (item.value) item.value.days = []; void load() }
 ...
 <AppShell title="日志归档">
  <div class="archive-page">
   ...
   <template v-if="item">
    ...
    <section v-if="item.status.reconciliation" class="panel"><div class="section-title"><h3>最近一次完整性对账 · {{ item.status.reconciliation.date }}</h3><el-tag :type="item.status.reconciliation.state === 'matched' ? 'success' : 'warning'">{{ checkLabels[item.status.reconciliation.state] }}</el-tag></div><p class="secondary">源库 {{ item.status.reconciliation.source_rows.toLocaleString() }} 条 · 目标库 {{ item.status.reconciliation.target_rows.toLocaleString() }} 条 · {{ date(item.status.reconciliation.finished_at) }}</p><p v-if="item.status.reconciliation.error" class="secondary">{{ item.status.reconciliation.error }}</p><p class="secondary">比较当日原始明细的全部字段，不修改数据或重算统计。对账结束后，退出对账模式再启用归档。</p></section><section class="panel daily-panel"><div class="section-title"><div><h3>每日归档任务</h3><span class="secondary">北京时间 · 同一天持续更新一条记录，不改变归档执行频率</span></div><el-date-picker v-model="month" @change="changeMonth" type="month" value-format="YYYY-MM" format="YYYY 年 MM 月" :clearable="false" aria-label="归档月份" style="width:170px" /></div>
     <el-table v-mobile-cards :data="item.days || []" row-key="date" empty-text="该月份暂无已上报的归档记录" v-loading="loading">
...
<style scoped>
.archive-page{min-width:0;display:grid;gap:12px}.intro{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:0}.eyebrow{font-size:11px;letter-spacing:1.5px;color:var(--ct-ink-2)}.intro h2{font-size:20px;margin:0 0 4px}.intro p,.setup p,.dialog-note{font-size:13px;line-height:1.8;color:var(--ct-ink-2);margin:0}.panel{min-height:0;min-width:0;background:var(--ct-surface);border:1px solid var(--ct-line);border-radius:10px;padding:16px}.status-panel{display:flex;align-items:center;justify-content:space-between;gap:20px}.status-line{display:flex;align-items:center;gap:12px;margin:0}.status-panel p{margin:6px 0 0}.actions{display:flex;flex-shrink:0}.label{font-size:13px;color:var(--ct-ink-2)}.secondary{font-size:12px;color:var(--ct-ink-3);line-height:1.8}.metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.metrics .panel{display:flex;flex-direction:column;gap:6px}.metrics strong{font-size:22px;font-weight:600;overflow-wrap:anywhere;font-variant-numeric:tabular-nums}.metrics small{font-size:13px;font-weight:400;color:var(--ct-ink-3)}.green{color:var(--ct-ok)}.section-title{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}h3{font-size:15px;margin:0 0 6px}.policy{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.policy div{display:flex;flex-direction:column;gap:10px}.policy span{font-size:12px;color:var(--ct-ink-3)}.policy b{font-size:14px;font-weight:500}.setup{padding:22px 24px;border:1px solid var(--ct-line);background:var(--ct-surface-2);border-radius:12px}.setup p{margin-top:8px}code{font:12px Consolas,monospace}.dialog-note{margin-bottom:20px}@media(max-width:850px){.metrics{grid-template-columns:1fr}.policy{grid-template-columns:repeat(2,1fr)}.status-panel,.section-title{align-items:flex-start;flex-direction:column}.intro{align-items:flex-start}}
</style>
