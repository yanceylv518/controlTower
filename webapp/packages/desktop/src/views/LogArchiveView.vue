...
import ArchiveTaskOverview from '../components/ArchiveTaskOverview.vue'
...
import {
  toggleArchiveGroup, archiveDataLabel, type ArchiveTaskGroup, beijingDate, buildArchiveDays, buildWorkflowDays, canCheckArchiveDay, formatArchiveCount, archiveExecution, archiveWorkflowLabels, archivePreparationActivity, archiveWorkflowOperation, archiveDiagnosticReason,
...
const originInitialized = ref(false)
const calendarOrigin = computed(() => item.value?.status.calendar_origin)
const firstDate = computed(() => calendarOrigin.value?.date || '')
function disabledArchiveMonth(date: Date) { return !!firstDate.value && `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}` < firstDate.value.slice(0,7) }
const monthReady = computed(() => loadedMonth.value === month.value && !!item.value)
const allMonthDays = computed(() => monthReady.value ? (fullHistory.value && item.value ? buildWorkflowDays(month.value, item.value, today.value) : buildArchiveDays(month.value, item.value?.days || [], today.value, item.value?.status.reconciliation)) : [])
const days = computed(() => calendarOrigin.value?.source === 'empty' ? [] : allMonthDays.value.filter(day => !firstDate.value || day.date >= firstDate.value))
...
const calendarOffset = computed(() => {
  const first = new Date(`${days.value[0]?.date || month.value+'-01'}T12:00:00+08:00`)
...
function showDay(day: ArchiveCalendarDay) { if (day.kind !== 'future') { tab.value = 'daily'; detailDate.value = day.date; detailTab.value = 'statistics' } }
...
async function load() {
  ...
  try {
    ...
    failure.value = ''
    if (firstDate.value && (!originInitialized.value || month.value < firstDate.value.slice(0,7))) {
      originInitialized.value = true
      const initialMonth = firstDate.value.slice(0,7)
      if (month.value !== initialMonth) { month.value = initialMonth; changeMonth() }
    }
  } catch (error) {
...
function toggleGroup(group: ArchiveTaskGroup) {
  if (item.value && writable.value && online.value && versionApplied.value && chosenReady.value && item.value.enabled) void save(toggleArchiveGroup(item.value.config, group), item.value.site_id)
}
 ...
 <AppShell title="日志归档">
  <div class="archive-page">
    <div class="archive-navigation">
      <el-tabs v-if="item" v-model="tab" class="archive-tabs" aria-label="归档工作区"><el-tab-pane label="运行总览" name="overview" /><el-tab-pane label="每日数据" name="daily" /><el-tab-pane label="设置" name="tasks" /></el-tabs>
      <span v-else class="secondary">按北京时间查看归档上报与对账结果</span>
      <div class="navigation-actions">
        <div v-if="item" class="archive-state-group" aria-live="polite"><el-tooltip :content="`最近非空批次：${displayDate(item.status.last_success)}；点击查看设置`" placement="bottom"><button type="button" class="archive-state" :aria-label="`归档状态：${failure ? '状态待刷新' : state.text}，查看设置`" @click="tab = 'tasks'"><el-tag :type="failure ? 'warning' : state.type">{{ failure ? '状态待刷新' : state.text }}</el-tag></button></el-tooltip><span v-if="!item.enabled" class="secondary">站点未启用</span></div>
        ...
        <div v-if="item && (tab === 'daily' || (tab === 'overview' && !fullHistory))" class="month-control"><el-date-picker v-model="month" type="month" value-format="YYYY-MM" format="YYYY 年 MM 月" :clearable="false" :disabled-date="disabledArchiveMonth" aria-label="归档月份" @change="changeMonth" /></div>
    ...
    <template v-if="item">
      <section v-if="fullHistory && tab === 'overview'" class="archive-connection" aria-label="执行连接状态"><span>执行 Agent：{{ item.config.agent_id || '未配置' }}</span><span>{{ online ? '在线' : '上报过期' }} · {{ displayDate(item.seen_at) }}</span><span>{{ versionApplied ? '配置已应用' : '等待配置确认' }}</span></section>
      <ArchiveTaskOverview v-if="fullHistory && tab === 'overview' && item.config.pipeline" :item="item" :days="days" :month="month" :writable="writable && online && versionApplied && chosenReady && item.enabled" :stale="!!failure || !online" @toggle-group="toggleGroup" @daily="tab = 'daily'" @select="showDay" />
      <ArchiveWorkflowProgress v-if="fullHistory && !item.config.pipeline && tab === 'overview'" :item="item" :now="now" :read-failed="!!failure" />
      <ArchivePipelineTasks v-if="fullHistory && tab === 'tasks'" mode="manage" :item="item" :supported="capabilities.four_tasks" :writable="writable && online && versionApplied" :stale="!!failure || !online" @save="savePipeline" @master-toggle="toggle" />
      ...
      <ArchiveDailyCalendar :origin="calendarOrigin" :origin-error="item.status.calendar_origin_error" v-if="fullHistory && tab === 'daily' && dailyView === 'calendar'" :days="days" :month="month" :current-date="item.status.pipeline?.active.verification?.date || item.status.pipeline?.active.organization?.date || item.status.workflow?.date" :loading="loading && !monthReady" :paused="!item.config.running" :stale="!!failure || !online" @select="showDay" />
      ...
      <section v-if="tab === 'daily' && dailyView === 'list'" class="panel daily-panel"><div class="section-title daily-heading"><div><h3>每日数据</h3><p class="secondary">{{ fullHistory ? '按北京时间查看每天最近上报的状态；月表已有数量与整理中的统计分开显示' : '全月日期均保留，条数为已归档累计值' }}</p></div><el-radio-group v-model="dayFilter" size="small" aria-label="日期筛选"><el-radio-button value="all">全部</el-radio-button><el-radio-button value="reported">已上报</el-radio-button><el-radio-button value="pending">未上报 / 异常</el-radio-button></el-radio-group></div><p v-if="fullHistory" class="secondary">当前任务：{{ execution?.title }}。{{ execution?.detail }} 明细按最多 100 个日期分批采集，约每 30 秒读取一页；页面刷新不会扫描源库。</p><el-alert v-if="fullHistory && item.status.workflow_daily_error" :title="`每日明细读取失败：${archiveDiagnosticReason(item.status.workflow_daily_error).reason}（${item.status.workflow_daily_error}）。以下保留最近记录。`" type="warning" :closable="false" show-icon /><el-alert v-else-if="fullHistory && !item.status.workflow_daily" title="尚未收到新版每日明细；需配套更新 Server 和 Agent 后自动上报。没有明细不表示日志为零。" type="info" :closable="false" show-icon /><p v-if="dayFilter === 'pending'" class="secondary">只显示已结束且无累计上报，或最近处理异常的日期。</p>
        <el-table v-mobile-cards v-loading="loading && !monthReady" :data="visibleDays" row-key="date" empty-text="当前月份没有符合筛选条件的日期">
          ...
          <el-table-column :label="fullHistory ? '最近处理状态' : '日志状态'" min-width="146"><template #default="{ row }"><el-tag :type="row.tagType" size="small">{{ fullHistory ? archiveDataLabel(row.kind) : row.label }}</el-tag></template></el-table-column>
  ...
  <section v-if="detail && detailOpen && tab === 'daily'" class="panel archive-inline-detail" aria-label="日期数据详情">
    <div class="section-title"><h3>{{ detailDate }} · {{ fullHistory ? archiveDataLabel(detail.kind) : detail.label }}</h3><el-button text @click="detailOpen = false">收起</el-button></div>
    <div class="detail-metadata"><span>{{ siteName }}</span><span>归档日志 {{ formatArchiveCount(fullHistory ? detail.raw?.rows : detail.counts?.log_rows) }} 条</span><span>数量更新于 {{ displayDate(detail.raw?.observed_at) }}</span></div>
    <div v-if="isIssue(detail)" class="day-failure"><b>本次处理未完成</b><p>{{ detail.reason }}</p><details v-if="detail.workflow?.diagnostic"><summary>技术详情</summary><p>操作：{{ detail.workflow.diagnostic.operation?.code }}；处理表：{{ detail.workflow.diagnostic.operation?.table || '—' }}；日志 ID：{{ detail.workflow.diagnostic.row_id || '—' }}；MySQL：{{ detail.workflow.diagnostic.mysql_number || '—' }} / {{ detail.workflow.diagnostic.sql_state || '—' }}</p></details></div>
    <p v-else class="secondary">{{ detail.reason }}</p>
    <p v-if="detail.raw?.error_code" class="error-text">数量刷新失败：{{ archiveDiagnosticReason(detail.raw.error_code).reason }}。已有数量保留原统计时间。</p>
    <el-tabs v-model="detailTab" aria-label="日期数据详情视图">
      <el-tab-pane label="日统计" name="statistics"><el-empty description="当前接口尚未提供多维日统计" :image-size="54" /><p class="secondary">日统计按用户、令牌、模型、渠道、历史计价和用户折扣分组。旧总量记录不作为新版账单依据。</p></el-tab-pane>
      <el-tab-pane label="日志明细" name="logs"><el-empty description="当前归档接口尚未提供逐条日志查询" :image-size="54" /><p class="secondary">需要读取该日归档月表，展示每条请求实际适用的输入、输出、缓存单价和折扣。此处不以源库查询替代归档明细。</p></el-tab-pane>
    </el-tabs>
    <details class="technical-details"><summary>现有处理记录与核验依据</summary>
      <div class="detail-counts"><div><span>已统计原始日志</span><b>{{ formatArchiveCount(detail.counts?.log_rows) }}</b></div><div><span>请求日志</span><b>{{ formatArchiveCount(detail.counts?.request_rows) }}</b></div><div><span>错误请求</span><b>{{ formatArchiveCount(detail.counts?.error_rows) }}</b></div></div>
      <p>月表：logs_{{ detailDate.slice(0,7).replace('-','') }} · 结果更新时间：{{ displayDate(detail.workflow?.updated_at) }}</p>
      <p v-if="detail.reconciliation">源库 {{ numberCount(detail.reconciliation.source_rows) }} 条 · 目标库 {{ numberCount(detail.reconciliation.target_rows) }} 条 · {{ detail.reconciliation.error || '保留最近核验记录' }}</p>
      <ArchiveStatusHelp v-if="fullHistory" :kind="detail.kind" />
    </details>
    <el-button v-if="!fullHistory" :disabled="!!checkReason || !canCheckArchiveDay(detail.date, today, item?.config.delay_seconds ?? 0, now)" :title="checkReason" @click="openCheck(detailDate)">按日对账</el-button>
  </section>
  ...
  <el-dialog v-model="dialog" title="执行配置" width="min(540px, calc(100vw - 24px))" :close-on-click-modal="false">
