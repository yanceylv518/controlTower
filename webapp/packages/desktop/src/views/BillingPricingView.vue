...
const priceForm = reactive({ model_name: "", effective_from: new Date().toISOString().slice(0, 10), tiers: [{ tier_from: 0, input_price: "0", output_price: "0", cache_price: "0", cache_write_price: "0" }] });
...
function editPrice(item: BillingModelItem) { selected.value=item; priceForm.model_name=item.model_name; priceForm.effective_from=new Date().toISOString().slice(0,10); priceForm.tiers=[{tier_from:0,input_price:item.input_price||"0",output_price:item.output_price||"0",cache_price:item.cache_price||"0",cache_write_price:item.cache_write_price||"0"}]; priceOpen.value=true; }
function addTier(){const last=priceForm.tiers.at(-1)!;priceForm.tiers.push({tier_from:Number(last.tier_from)+1,input_price:last.input_price,output_price:last.output_price,cache_price:last.cache_price,cache_write_price:last.cache_write_price});}
  ...
  <AppShell title="模型管理">
    ...
    <AsyncPanel :loading="state.loading.value" :error="state.error.value" :empty="!visible.length" @retry="state.reload">
      <el-table :data="visible" row-key="model_name">
        ...
        <el-table-column label="缓存写入价格" min-width="125"><template #default="s">{{ formatPrice(s.row.cache_write_price) }}</template></el-table-column>
