...
const createForm = ref<{ instance_id: string; bill_type: "user" | "upstream"; user_id?: number; upstream_id?: number; exclude_zero_output: boolean; recalculate: boolean; range: [Date, Date] | [] }>({ instance_id: "", bill_type: "user", exclude_zero_output: false, recalculate: false, range: [] });
const state = useAsyncData(async () => {
  await filters.loadInstances();
  if (!filters.site_id) return { items: [] as BillingJob[], pricing_source_selection: false };
...
async function createJob() {
  ...
  const duplicate = items.value.some((job) => job.status !== "failed" && job.instance_id === form.instance_id && job.job_type === wantedType
  ...
  creating.value = true;
  try {
    await dashboard.createBillingStatement({instance_id:form.instance_id,statement_type:form.bill_type,user_id:form.bill_type==="user"?form.user_id:undefined,upstream_id:form.bill_type==="upstream"?form.upstream_id:undefined,from:wantedFrom,to:wantedTo,exclude_zero_output:form.exclude_zero_output,recalculate:form.recalculate});
...
<template>
  <AppShell title="账单任务">
    ...
    <el-alert v-if="counts.active" type="warning" :title="counts.running ? `当前 1 个任务正在执行，${counts.queued} / 5 个任务排队等待` : `当前有 ${counts.queued} / 5 个任务等待执行`" description="系统同一时间只执行一个账单任务，等待队列最多 5 个；相同站点、类型、对象、账期和计费选项的任务不能重复创建。" :closable="false" show-icon />
    ...
    <el-dialog v-model="createVisible" title="创建账单任务" width="560px">
      <el-form label-width="100px">
        ...
        <el-form-item label="计费来源"><el-radio-group v-model="createForm.recalculate"><el-radio :value="false">NewAPI 计费结果（默认）</el-radio><el-radio :value="true">重新计费</el-radio></el-radio-group><small>{{ createForm.recalculate ? "按日志记录的单价、倍率和表达式重新计算；核对差异仍计入账单。" : "直接采用 NewAPI 日志实际扣费，不重算；上游渠道折扣另行应用。" }}</small></el-form-item>
        <el-form-item label="订单范围"><el-checkbox v-model="createForm.exclude_zero_output">排除输出 Token 为 0 的订单</el-checkbox></el-form-item>
        <el-alert type="info" :closable="false" title="每次任务都会完整生成一条新账单；相同站点、账单类型、对象、账期和计费选项的任务或账单已经存在时，不能重复创建。" />
      </el-form>
      <template #footer><el-button @click="createVisible=false">取消</el-button><el-button type="primary" :loading="creating" :disabled="!state.data.value?.pricing_source_selection || counts.queued >= 5 || !createForm.instance_id || createForm.range.length !== 2 || (createForm.bill_type === 'user' ? !createForm.user_id : !createForm.upstream_id)" @click="createJob">{{ counts.queued >= 5 ? '等待队列已满' : '创建任务' }}</el-button></template>
    </el-dialog>
  </AppShell>
</template>
