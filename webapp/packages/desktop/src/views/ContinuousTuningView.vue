  ...
  <el-drawer v-model="helpOpen" title="调权中心使用说明" size="560px" class="tuning-help">
    <div class="help-guide">
      ...
      <section><h3>规则参数怎么用</h3><dl class="parameter-guide">
        ...
        <div><dt>写入死区</dt><dd>计算权重相对上次已写权重的变化达到基础权重的该百分比，才视为需要常规写入；0 表示只要整数权重变化 1 就满足。可抑制滑动窗口造成的微小抖动。</dd></div>
        <div><dt>最小写入间隔</dt><dd>同一渠道两次常规调权写入之间至少等待该分钟数。常规写入必须同时满足死区和间隔；熔断置零、恢复软启动和写失败后的重试不受此间隔限制。</dd></div>
      ...
      <section><h3>安全保护</h3><ul><li>数据不足时保持当前权重，不用不完整样本做决策。</li><li>常规调权只有同时达到写入死区并超过最小写入间隔才写入 new-api；熔断、恢复和暂停重试不受限制。</li><li>检测到人工修改时自动暂停，优先尊重人工设置。</li><li>写入 new-api 连续失败 3 次自动暂停并记录原因，之后每 10 分钟重试一次，恢复成功自动解除。</li><li>所有自动写入、熔断、探测和恢复都记录在“变更记录”。</li></ul></section>
