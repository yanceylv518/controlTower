...
const phaseText = (state?: TuningContinuousState) => {
  if (!state) return "等待评估";
  if (state.paused_reason === "manual_override") return "人工接管暂停";
  if (state.paused_reason) return state.paused_reason;
  if (state.phase === "circuit") return `熔断（下次探测 ${state.next_probe_at ? formatTime(state.next_probe_at) : "待定"}）`;
  if (state.phase === "probing") return `恢复探测 ${state.probe_attempts || 0}/${policy.continuous.probe_count}`;
  if (state.phase === "soft_start") return "恢复软启动";
  return "正常调度";
};
const phaseType = (state?: TuningContinuousState) => state?.phase === "circuit" ? "danger" : state?.phase === "probing" || state?.phase === "soft_start" || state?.paused_reason ? "warning" : "success";
const eventName = (rule: string) => ({ weight_write: "自动写权", manual_takeover: "人工接管", auto_paused: "自动暂停", circuit_opened: "触发熔断", probe_started: "开始恢复探测", probe_failed: "探测未通过", circuit_recovered: "恢复软启动" } as Record<string, string>)[rule] || rule;
...
async function save() {
  saving.value = true;
  try { bases.value = (await dashboard.saveTuningBaseValues(siteID.value, bases.value)).items; await dashboard.saveTuningPolicy(siteID.value, policy, mode.value); ElMessage.success("连续调度配置已保存，将在下一分钟评估生效"); await load(); }
...
<template>
  <AppShell title="调权中心"><div v-loading="loading" class="page">
    ...
    <el-card shadow="never"><template #header><div class="title"><strong>1. 渠道总览与基础值</strong><span>当前站点：{{ siteID || "无" }}</span></div></template>
    ...
    <el-card shadow="never"><template #header><strong>2. 连续调度参数</strong></template>
      ...
      <el-button class="save" type="primary" :loading="saving" :disabled="!siteID" @click="save">保存连续调度配置</el-button>
    </el-card>
