...
const state = useAsyncData(async () => {
  ...
  const items:BillingUserSummary[]=users.items.map((user)=>({user_id:user.id,username:user.display_name||user.username,request_count:0,prompt_tokens:0,completion_tokens:0,cache_tokens:0,cache_write_tokens:0,quota:0,amount:"0.000000",balance:user.quota,unpriced_models:[],price_sources:[]}));
...
<template>
  <AppShell title="用户账单">
    ...
    <el-drawer v-model="detailOpen" :title="`${selected?.username || '用户'} · ${generationRange[0]} 至 ${generationRange[1]}`" size="78%">
      <div class="drawer-actions"><el-button :loading="Boolean(selected && exporting[selected.user_id])" @click="selected && exportUser(selected)">{{ selected && exporting[selected.user_id] ? "后台生成中" : "导出完整账单 Excel" }}</el-button><el-button tag="a" :href="anomalyExportURL">导出异常订单</el-button><router-link :to="`/readonly-logs?username=${encodeURIComponent(selected?.username || '')}&from=${generationRange[0]}&to=${generationRange[1]}`"><el-button>查看使用日志</el-button></router-link></div>
      <AsyncPanel :loading="detail.loading.value" :error="detail.error.value" :empty="!detail.data.value?.items?.length" @retry="detail.reload"><el-table :data="detail.data.value?.items || []" class="billing-detail-table"><el-table-column prop="day" label="日期" width="110" /><el-table-column prop="model_name" label="模型" min-width="160" /><el-table-column label="档位" width="80"><template #default="s">{{ s.row.tier_from > 0 ? `≥${s.row.tier_from}` : "—" }}</template></el-table-column><el-table-column prop="group_name" label="分组" min-width="90" /><el-table-column prop="request_count" label="请求" width="75" align="right"><template #default="s">{{ formatNumber(s.row.request_count) }}</template></el-table-column><el-table-column prop="prompt_tokens" label="普通输入" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.prompt_tokens) }}</template></el-table-column><el-table-column prop="cache_tokens" label="缓存读取" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.cache_tokens) }}</template></el-table-column><el-table-column prop="cache_write_tokens" label="缓存写入" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.cache_write_tokens) }}</template></el-table-column><el-table-column prop="completion_tokens" label="输出 Token" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.completion_tokens) }}</template></el-table-column><el-table-column label="输入价/1M" width="115" align="right"><template #default="s">{{ unitPrice(s.row.input_price) }}</template></el-table-column><el-table-column label="读取价/1M" width="115" align="right"><template #default="s">{{ unitPrice(s.row.cache_price) }}</template></el-table-column><el-table-column label="写入价/1M" width="115" align="right"><template #default="s">{{ unitPrice(s.row.cache_write_price) }}</template></el-table-column><el-table-column label="输出价/1M" width="115" align="right"><template #default="s">{{ unitPrice(s.row.output_price) }}</template></el-table-column><el-table-column prop="amount" label="金额" width="110" align="right"><template #default="s">{{ s.row.unpriced ? "无法取价" : money(s.row.amount) }}</template></el-table-column></el-table></AsyncPanel>
    </el-drawer>
  </AppShell>
</template>
<style scoped>.billing-total{display:flex;align-items:center;gap:28px;padding:12px 4px}.billing-total b{font-size:18px;color:var(--el-color-primary);font-variant-numeric:tabular-nums}.billing-period{padding-right:20px;border-right:1px solid var(--el-border-color)}.billing-period b{font-size:14px}.billing-period small{display:block;margin-top:3px;color:var(--el-text-color-secondary)}.pending-alert{margin-bottom:10px}.billing-table :deep(.el-table__row){cursor:pointer}.billing-table :deep(.cell),.billing-detail-table :deep(.cell){font-variant-numeric:tabular-nums}.sub{display:block;color:var(--el-text-color-secondary);margin-top:3px}.drawer-actions{display:flex;gap:8px;justify-content:flex-end;margin-bottom:12px}.job-progress{color:var(--el-color-primary);font-variant-numeric:tabular-nums}.period-label{color:var(--el-text-color-secondary);font-size:13px}</style>
