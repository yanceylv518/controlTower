...
<template>
  <AppShell title="用户账单">
    ...
    <el-drawer v-model="detailOpen" :title="`${selected?.username || '用户'} · ${month}`" size="72%"><div class="drawer-actions"><el-button tag="a" :href="detailExportURL">导出明细 CSV</el-button><router-link :to="`/readonly-logs?username=${encodeURIComponent(selected?.username || '')}&month=${month}`"><el-button>查看使用日志</el-button></router-link></div><AsyncPanel :loading="detail.loading.value" :error="detail.error.value" :empty="!detail.data.value?.items.length" @retry="detail.reload"><el-table :data="detail.data.value?.items || []"><el-table-column prop="day" label="日期" width="110" /><el-table-column prop="model_name" label="模型" min-width="180" /><el-table-column label="档位" width="90"><template #default="s">{{ s.row.tier_from > 0 ? `≥${s.row.tier_from}` : "—" }}</template></el-table-column><el-table-column prop="group_name" label="分组" min-width="100" /><el-table-column prop="request_count" label="请求" width="80" /><el-table-column prop="prompt_tokens" label="输入" min-width="100" /><el-table-column prop="completion_tokens" label="输出" min-width="100" /><el-table-column prop="amount" label="金额" width="110"><template #default="s">{{ s.row.unpriced ? "无法取价" : money(s.row.amount) }}</template></el-table-column><el-table-column prop="price_source" label="价格来源" width="100" /></el-table></AsyncPanel></el-drawer>
  </AppShell>
</template>
