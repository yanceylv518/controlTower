...
type Watch={id:string;site:string;user_id:number;token_id:number;label:string;rule:'first'|'resume';gap_minutes:number;include_failed:boolean;phone:boolean;message:boolean;person_ids:string[];enabled:boolean;revision:number;round:number;started_at:string;last_at:string;last_id:number;fired:boolean;reset?:boolean}
...
async function edit(w?:Watch){draft.value=w?{...w,person_ids:[...w.person_ids]}:blank();mode.value=w?.token_id?'key':'account';editor.value=true;identityError.value='';await loadUsers();if(draft.value.user_id)await loadKeys(false)}
async function saveWatch(enabled:boolean){
 ...
 const site=filters.site_id,url=endpoint(),body=JSON.stringify({action:'watch',watch:{...draft.value,enabled,token_id:mode.value==='account'?0:draft.value.token_id}});saving.value=true
...
<template>
 <AppShell title="测试跟进">
  ...
  <div v-loading="loading" class="trial-page">
   ...
   <template v-if="data"><div class="trial-summary"><span>{{ stateText }}</span><span>最近检测：{{ stamp(data.checked_at) }}</span><span>站点通知名称：{{ data.display_name||filters.site_id }}</span></div>
   ...
   <el-table v-if="tab==='watches'" v-mobile-cards :data="data.watches" empty-text="暂无测试对象，添加后开始观察后续调用">
    ...
    <el-table-column label="检测状态"><template #default="{row}"><el-tag :type="!row.enabled?'info':row.fired?'success':'primary'">{{ !row.enabled?'暂停 / 草稿':row.fired?'已发现调用':'等待调用' }}</el-tag></template></el-table-column>
   ...
   <template v-else><div><el-button link @click="editor=false">← 返回测试名单</el-button></div><div class="trial-editor"><el-form class="panel sub-panel" label-position="top" :disabled="saving">
...
<style scoped>.trial-page{display:grid;gap:14px;min-width:0}.trial-summary{display:flex;gap:16px;flex-wrap:wrap;font-size:12px;color:var(--ct-ink-3)}.trial-editor{display:grid;grid-template-columns:minmax(0,1.4fr) minmax(0,1fr);gap:16px;align-items:start}.trial-editor>.panel{margin:0;min-width:0}.trial-editor :deep(.el-select){width:100%}.trial-preview p{line-height:1.9;overflow-wrap:anywhere}.trial-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:16px}@media(max-width:900px){.trial-editor{grid-template-columns:1fr}}</style>
