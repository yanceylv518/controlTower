  ...
  return [
    {
      ...
      icon: CircleCheck,
      tone: success < 0.95 ? "danger" : success < 0.99 ? "warning" : "success",
    },
    {
      label: "错误率",
      value: formatPercent(m?.error_rate),
      icon: CircleClose,
      tone: error > 0.05 ? "danger" : error > 0.01 ? "warning" : "success",
    },
    {
      label: "请求数",
    ...
    {
      label: "P95 耗时",
      value: formatSeconds(m?.p95_use_time),
      icon: Stopwatch,
      tone: "",
    },
    ...
    {
      label: "健康检查",
      value: `${runtime?.health.up_count ?? 0} 正常 / ${runtime?.health.down_count ?? 0} 异常`,
      icon: Monitor,
      tone: (runtime?.health.down_count || 0) > 0 ? "danger" : "success",
    },
    {
      label: "容器",
      value: `${runtime?.docker.running_count ?? 0} 运行 / ${runtime?.docker.stopped_count ?? 0} 停止`,
      icon: Monitor,
      tone: (runtime?.docker.stopped_count || 0) > 0 ? "danger" : "success",
    },
  ];
});
function dimensionRoute(item: AlertItem) {
  const path =
    item.dimension_type === "channel" ||
    item.dimension_type === "instance_channel"
      ? "/channels"
      : item.dimension_type === "model" ||
          item.dimension_type === "instance_model"
        ? "/models"
        : item.dimension_type === "user" ||
            item.dimension_type === "instance_user"
          ? "/customers"
          : "";
  return path ? { path, query: { key: item.dimension_key } } : "/alerts";
}
async function refresh(silent = false) {
  if (document.visibilityState !== "visible") return;
  const background = silent && overview.value !== null;
  if (!background) {
    loading.value = true;
    error.value = "";
  }
  ...
  <AppShell title="运行总览">
    ...
    <section v-loading="loading">
      <el-alert
        ...
        @close="error = ''"
      /><el-alert
        v-if="filters.loaded && !filters.instances.length"
        class="instance-guide"
        type="info"
        :closable="false"
        show-icon
        ><template #title
          >尚未创建实例——<router-link to="/instances"
            >前往实例管理创建并部署 Agent</router-link
          ></template
        ></el-alert
      >
      <div class="kpi-grid">
        ...
        <section class="panel alerts-panel">
          ...
          <div v-if="alerts.length" class="alert-list">
            <article v-for="item in alerts" :key="item.id" class="alert-item">
              ...
              >
              <div>
                <strong :title="item.display_key">{{
                  (item.display_key || item.title).replace(/ \(ID \d+\)$/, "")
                }}</strong>
                <p>{{ item.summary }}</p>
          ...
          <el-empty v-else description="当前没有活跃告警" />
        </section>
      </div></section
  ></AppShell>
</template>
