  ...
  <AppShell title="渠道账单">
    ...
    <el-drawer v-model="detailOpen" :title="`${selected?.channel_name || `渠道 ${selected?.channel_id || ''}`} · ${generationRange[0]} 至 ${generationRange[1]}`" size="82%">
      ...
      <AsyncPanel :loading="detail.loading.value" :error="detail.error.value" :empty="!detail.data.value?.details?.length" empty-text="该渠道在所选区间没有日账单数据" @retry="detail.reload">
        <el-table :data="detail.data.value?.details || []" class="channel-detail-table">
          <el-table-column prop="day" label="日期" width="110" /><el-table-column prop="model_name" label="模型" min-width="160" /><el-table-column label="档位" width="85"><template #default="s">{{ s.row.tier_from > 0 ? `≥${formatNumber(s.row.tier_from)}` : "—" }}</template></el-table-column><el-table-column prop="group_name" label="分组" min-width="90" />
          <el-table-column prop="request_count" label="请求" width="85" align="right"><template #default="s">{{ formatNumber(s.row.request_count) }}</template></el-table-column><el-table-column prop="abnormal_rows" label="异常" width="75" align="right"><template #default="s">{{ formatNumber(s.row.abnormal_rows) }}</template></el-table-column><el-table-column prop="prompt_tokens" label="普通输入" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.prompt_tokens) }}</template></el-table-column><el-table-column prop="cache_tokens" label="缓存读取" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.cache_tokens) }}</template></el-table-column><el-table-column prop="cache_write_tokens" label="缓存写入" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.cache_write_tokens) }}</template></el-table-column><el-table-column prop="completion_tokens" label="输出" min-width="105" align="right"><template #default="s">{{ formatNumber(s.row.completion_tokens) }}</template></el-table-column>
          <el-table-column label="输入价/1M" width="110" align="right"><template #default="s">{{ unitPrice(s.row.input_price) }}</template></el-table-column><el-table-column label="读取价/1M" width="110" align="right"><template #default="s">{{ unitPrice(s.row.cache_price) }}</template></el-table-column><el-table-column label="写入价/1M" width="110" align="right"><template #default="s">{{ unitPrice(s.row.cache_write_price) }}</template></el-table-column><el-table-column label="输出价/1M" width="110" align="right"><template #default="s">{{ unitPrice(s.row.output_price) }}</template></el-table-column>
          <el-table-column label="原价" width="105" align="right"><template #default="s">{{ s.row.unpriced ? "无法取价" : money(s.row.amount) }}</template></el-table-column><el-table-column label="折后金额" width="110" align="right"><template #default="s"><b>{{ s.row.unpriced ? "—" : discountedMoney(s.row.amount) }}</b></template></el-table-column>
        </el-table>
      </AsyncPanel>
    </el-drawer>
  </AppShell>
...
<style scoped>
.page{display:flex;height:calc(100vh - 78px);min-height:0;flex-direction:column;gap:10px;overflow:hidden}.period-summary{display:flex;align-items:center;padding:8px 12px;border:1px solid var(--el-border-color-lighter);border-radius:6px;background:var(--el-fill-color-blank);color:var(--el-text-color-secondary)}.period-summary b{margin-left:8px;color:var(--el-text-color-primary)}.actual-period{margin-left:auto;font-size:12px}.actual-period strong{margin-left:6px;color:var(--el-text-color-primary)}.content{min-height:0;flex:1}.table-wrap{height:calc(100% - 58px);min-height:240px}.table-wrap :deep(.cell),.channel-detail-table :deep(.cell){font-variant-numeric:tabular-nums}.channel-table :deep(.el-table__row){cursor:pointer}.sub{display:block;margin-top:3px;color:var(--el-text-color-secondary)}.period-label{color:var(--el-text-color-secondary);font-size:13px}.drawer-summary{display:flex;justify-content:flex-end;gap:28px;margin-bottom:12px;padding:10px 12px;background:var(--el-fill-color-light);border-radius:6px}.drawer-summary b{margin-left:6px;color:var(--el-color-primary);font-variant-numeric:tabular-nums}
</style>
