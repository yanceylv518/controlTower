...
const verificationStatusText = (status?: string) => ({ pending: "等待中", running: "核对中", complete: "已完成", failed: "失败" }[status || ""] || status || "");
...
async function startVerification() {
  ...
  try {
    ...
    await verification.reload();
    scheduleVerificationPoll();
    ElMessage.success(result.reused ? "已恢复现有完整核对任务" : "完整核对任务已启动");
  } catch (error) { ElMessage.warning(billingTaskErrorMessage(error, "完整核对任务创建失败")); }
}
...
<template>
  <AppShell title="账单核对">
    ...
    <AsyncPanel v-else :loading="report.loading.value" :error="report.error.value" :empty="!report.data.value?.items.length" empty-text="该区间账单核对未发现差异" @retry="report.reload">
      ...
      <div v-if="report.data.value" class="cards">
        <div><span>账单金额</span><b>{{ money(report.data.value.totals.billing_amount) }}</b></div>
      ...
      <el-table :data="report.data.value?.items || []" :row-class-name="reconciliationRowClass">
        ...
        <el-table-column label="账单金额" min-width="130" align="right"><template #default="s">{{ money(s.row.billing_amount) }}</template></el-table-column>
    ...
    <section v-if="report.data.value" class="verification-panel">
      <div class="verification-heading">
        <div><h3>后台完整核对</h3><p>分页复扫该生成批次的 new-api 消费日志，并与 CT 正常账单、待确认记录逐日核对。</p></div>
      ...
      <el-alert v-if="verification.data.value?.job?.status === 'failed'" type="error" :closable="false" :title="verification.data.value.job.error_message || '完整核对失败，可重新启动任务'" />
      <el-empty v-else-if="!verification.data.value?.job" description="尚未启动完整核对" :image-size="80" />
    ...
    <el-drawer v-model="detailOpen" :title="`${selectedUser?.username || '用户'} · L2 日/模型/分组核对`" size="86%">
      <AsyncPanel :loading="detail.loading.value" :error="detail.error.value" :empty="!detail.data.value?.items.length" @retry="detail.reload">
        <el-table :data="detail.data.value?.items || []" :row-class-name="reconciliationRowClass">
          ...
          <el-table-column label="账单金额" min-width="120" align="right"><template #default="s">{{ money(s.row.billing_amount) }}</template></el-table-column>
    ...
    <el-drawer v-model="requestsOpen" :title="`${selectedScope?.day || ''} · ${selectedScope?.model_name || ''} · L3 请求核对`" size="88%">
      <AsyncPanel :loading="requests.loading.value" :error="requests.error.value" :empty="!requests.data.value?.items.length" @retry="requests.reload">
        ...
        <el-table :data="requests.data.value?.items || []" class="request-table">
          ...
          <el-table-column label="账单金额" min-width="110" align="right"><template #default="s">{{ money(s.row.billing_amount) }}</template></el-table-column>
