...
test('readonly logs mounts only the active responsive layout', () => {
  ...
  assert.match(source, /<tr v-for="\(view, index\) in renderedRows" v-memo=/)
  assert.match(source, /<article v-for="\(view, index\) in renderedRows" v-memo=/)
...
test('readonly logs exposes fallback requests and channel chains', () => {
  ...
  assert.match(source, /class="retry-chain-popover retry-chain-popover-layer"/)
  assert.match(source, /v-if="view\.retryChain"/)
  assert.match(source, /重试链路/)
  assert.doesNotMatch(source, /fallback-label/)
  assert.match(source, /const retryPopover = ref<RetryPopoverState \| null>\(null\)/)
  assert.match(source, /function positionRetryPopover\(\)/)
  assert.match(source, /class="retry-chain-popover retry-chain-popover-layer"/)
  assert.match(source, /z-index: 4000/)
  assert.match(source, /document\.addEventListener\('pointerdown', handleRetryPopoverPointerdown\)/)
  assert.match(source, /window\.addEventListener\('scroll', handleRetryPopoverViewportChange, true\)/)
