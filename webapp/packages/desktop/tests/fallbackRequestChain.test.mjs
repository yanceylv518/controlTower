...
test('unknown retry lookup stays inspectable without claiming a retry happened',async()=>{
  ...
  assert.equal((page.match(/v-else-if="view.retryUnknown">待确认/g)||[]).length,2)
  assert.match(page,/retryUnknown: admin && retryLookupUnknown\(row\)/)
})

test('fallback chains expand from the channel area without hijacking child controls', () => {
  const page = readFileSync(new URL('../src/views/ReadonlyLogsView.vue', import.meta.url), 'utf8')
  assert.match(page, /function toggleRetryChainFromChannelArea\(view: LogRowView, event: MouseEvent\)/)
  assert.match(page, /target\.closest\('\.col-channel, \.mobile-card-channel'\)/)
  assert.match(page, /target\.closest\('button, a, input, select, textarea, \[role="button"\]'\)/)
  assert.match(page, /class="log-row"[^>]*@click="toggleRetryChainFromChannelArea\(view, \$event\)"/)
  assert.match(page, /class="mobile-card-channel"[^>]*@click="toggleRetryChainFromChannelArea\(view, \$event\)"/)
  assert.match(page, /channel-badge.*@click\.stop="copyText/)
  assert.match(page, /channel-affinity-trigger.*@click\.stop="openChannelAffinity/)
  assert.match(page, /retry-chain-trigger.*@click\.stop="toggleRetryChain\(view\)/)
})
