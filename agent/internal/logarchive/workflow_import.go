...
func (w *Worker) importWorkflowPage(ctx context.Context, g af.WriterGrant, s *workflowState) error {
	...
	if s.Phase == "import_target" {
		...
		if s.Table != "" {
			...
			var engine string
			if err := w.target.QueryRowContext(ctx, `SELECT ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=?`, s.Table).Scan(&engine); err != nil || !strings.EqualFold(engine, "InnoDB") {
				return ErrFoundationSchema
			}
	...
	case "import_target":
		...
		for _, row := range batch.Rows {
			...
			if _, e = tx.ExecContext(ctx, `INSERT INTO archive_log_state(id,contribution,raw_row_hash,last_batch_id) VALUES(?,?,?,NULL)`, id, string(raw), hash[:]); e != nil {
				return errors.New("legacy archive contains duplicate ID or invalid contribution")
			}
			daily.add(c.Day, c, 1)
