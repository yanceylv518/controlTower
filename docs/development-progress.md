...
- 2026-08-10：调权直连 new-api B1 落地（design-tuning-direct-control.md）：站点级直连三件套配置（041 迁移，token AES-GCM 加密），directcontrol 包按站点分流——直连站点的 auto 写权重/熔断探针/预检由服务端直调 newapi 管理 API（命令行只落终态，agent/哨兵消费方零改动），未配置站点回落 Agent 命令队列；真库集成测试门控于 CT_MYSQL_TEST_DSN。
- 2026-07-17：健康检查历史拆分为独立小时级保留策略，默认保留 6 小时，清理任务每小时执行。
- 2026-07-17：系统状态健康检查改为最新状态摘要，历史检查默认折叠，减少周期性重复信息干扰。
- 2026-07-17：设置页改为响应式双列布局，宽屏取消固定窄容器，主配置与辅助配置并排展示，减少右侧空白和纵向滚动。
- 2026-07-29：v2.9-B1 值班调度引擎完成 observe-only 服务端实现；按模型优先级梯队评估、记录 demote/trial 建议并持久化假设调度状态，仍不执行渠道命令。
...
| v2.4-B1 Request ID 关联延时分诊 | 已规划 | 用 `instance_id + X-Oneapi-Request-Id` 将 Nginx 慢样本关联用户、渠道、模型和令牌 | 任务计划、009 迁移、慢样本关联 API 与 Web 筛选 | 生产已验证响应头与 new-api 使用日志 ID 一致；实现与验收待执行 |
...
| v2.9-B1 值班自动调度 observe | 已完成 | 按模型优先级梯队识别在岗渠道，生成降级/试岗建议但不执行命令 | 新 Policy、015 调度状态表、24h P95 基线、demote/trial outcome | 严重错误即时触发、持续窗口、混布跳过、备份三分支、退避/限额/重启恢复测试 |
...
| v2.3-B2 Web 打磨收尾 | 已完成 | `server/internal/dashboard/**`、`webapp/**` | 告警名称与维度跳转、区间聚合 KPI、样本格式、渠道搜索/状态分组/健康墙 |
...
## 2026-07-16：v2.4-B1 Request ID 关联延时诊断

- Agent 已从 Nginx timing 日志解析并上报慢样本 Request ID，兼容旧格式和无效字段。
- Server 已完成慢样本字段迁移、MySQL/内存存储以及 `log_samples` + `log_events` 批量精确关联。
- 延时分诊 API 与页面已支持用户、渠道、模型、令牌和关联状态展示/筛选，不使用时间邻近猜测。
- `go test ./...`、`go vet ./...`、前端测试和生产构建全部通过。
- 交付说明：`docs/v2.4-b1-delivery.md`。

## 2026-07-16：v2.7-B1 Web 体验修正

- Dashboard 指标接口新增纯名称 `display_name`，保留 `display_key` 兼容字段；维度、健康墙与告警展示不再把 ID 当主名称。
- 维度指标卡、比例条、总览 KPI 与趋势图已按走查反馈重排。
- 七处大列表已统一为默认 20 条的 20/50/100 分页，筛选会回到第一页；慢样本接口增量支持 `offset`。
- 延时分诊会自动选择在线优先实例；维度页完成 300px 左列独立滚动、吸顶、前 50 项渐进展开和窄屏适配。
- 交付说明：`docs/v2.7-b1-delivery.md`。

## 2026-07-16：v2.7-B3 稀疏指标曲线修正

- TrendChart 新增稀疏序列模式，TTFT 与缓存命中率的孤立点、跨空分钟连线可见，密集序列不变。
- 修复同一 1m 桶多次部分聚合时 `MergeMetric` 清空 TTFT P95 的根因，改为 NULL 安全 MAX 并覆盖三分支测试。
- 5m TTFT P95 使用子桶 P95 最大值作为保守上界近似，部分 NULL 与全 NULL 均有回归测试。
- 已核对 5m 大输入样本及缓存命中计数按可空加法汇总，读侧命中率计算正确。
- 交付说明：`docs/v2.7-b3-delivery.md`。

- 阶段状态只在有验证依据后更新。
- 每完成一个子项，在本文档对应表格更新状态和验证方式。
- 每次具体变更仍追加到根目录 `PROJECT_PROGRESS.md`。
- 2026-07-17：客户、渠道、模型监控详情页新增当前实例维度下拉快速切换；`pnpm typecheck` 与 `pnpm build` 已通过。
