...
**架构澄清（重要）**：系统只有一个 Agent、一个二进制——它同时具备"本地告警直发钉钉"和"上报监控数据到 Control Tower"两种能力，由配置决定开哪些：只配钉钉 webhook = 独立告警模式（当前生产）；加配 `CT_SERVER_URL` = 双模式（告警照发 + 数据上报，Server 故障不影响钉钉告警）；只配 Server = 纯上报。下文的"告警线/主线"仅指**功能批次的排期分组**，不是两个产品，永远不 fork。

v1.0 错误预警 Agent 已在生产运行并持续迭代（见 `iteration-log.md` v1.0.x 系列），产品主线（Server/Web/App）在其之上推进。
