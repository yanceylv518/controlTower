...
| 2 | — | 工程基础：CI 跑测试 + 构建 + 自动打部署包（tar.gz/LF/SHA256），根治 Windows 手工打包事故类（v1.0.2/3/4 三次踩坑的根源）；修掉剩余 P0 渠道快照常驻化 | M0 | 3~5 天 |
...
| 随时 | v1.1.x | 生产告警问题的穿插修复（如 v1.0.5/v1.0.6 模式），修完回主线 | 独立告警线 | — |
| 可选 | v1.2 | 在途请求检测（new-api 轻量中间件，轻度部分挂起的唯一可靠手段；需批准打破"不修改 new-api"边界） | 独立告警线 | 1 周 |
