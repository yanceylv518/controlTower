...
| 3 | v1.2 | 在途请求检测与请求参数采集：new-api 轻量中间件（一个 fail-safe 文件 + 两行注册，仅元数据不含 body 内容/密钥）产生请求开始/结束事件（含精确毫秒耗时、按 attempt 可见重试掩盖的失败），本地 JSON lines 文件，Agent tail 配对 → "在途超 120 秒连续 N 条"精确告警；不动 new-api 数据库 | 独立告警线 | ~1 周 |
| 4 | — | Server 产品化：认证、按实例 token、告警时间线、通知强化、渠道命令闭环，API 冻结；期间生产 Agent 保持独立模式不动 | M1 | 2~3 周 |
| 5 | v2.0 | Vue3 桌面 Web + 最小 Compose 部署；**汇合点**：生产 Agent 配置加 `CT_SERVER_URL` 进入双模式——钉钉直发告警保留为独立冗余链路（Server/Web 故障不影响告警），同时向 Server 上报供看板展示 | M2 + M4 最小部署 | 3~4 周 |
| 6 | v3.0 | 移动端 PWA App | M3 | 2~3 周 |
