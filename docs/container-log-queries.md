...
- 只有经文件身份与快照校验的完整时间索引，才能证明普通文件不与查询窗口重叠并跳过。
  不把修改时间或首条记录当作全文件时间边界，避免乱序日志漏查。首次仍分批建索引；
  gzip 在分批阶段顺序解压筛选，不在快照阶段预解压或探测文件头。
  快照仅保留目录元数据和每文件最多 256 字节的身份校验样本；扫描量统计为索引/筛选
  正文处理量，不包含目录操作、身份校验样本及 gzip 底层压缩读缓冲。
