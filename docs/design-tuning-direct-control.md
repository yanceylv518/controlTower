...
- **P2(B2 必修,最重要):直连写失败无告警**——引擎对写失败的处理是"本 tick 放弃、下 tick 重试",不落任何事件;命令路径的失败可见性靠"pending 命令过期→auto_paused 哨兵",直连行永不 pending 故哨兵永不触发。**直连站点若 token 被撤销/ALB 拒绝,auto 会静默停止生效**(计算权重与当前权重的差距在页面可见,但无主动告警)。B2 的"连续写失败→暂停+告警"就是补这个,若直连先于 B2 上生产,运维需人工留意调权中心的写入流水是否停更。
