...
- P3(2026-08-10 复盘修正结论):HTTP 写成功但落流水失败→newapi 已变而 CT 记账停留旧值——**对账回路会捕获,但捕获形态是打着"人工修改"错误标签的不可自愈暂停**(解除条件=快照回到旧值,永不满足,需人工处理)。触发窗口极窄:须恰好该笔事务失败而几毫秒后的状态落库成功(DB 整体故障时 tick 更早就中止);失败方向安全侧(停下来喊人)。缓解可选:wrapper 落流水失败重试一次,再败按"已写入"返回+大声记日志。
- P3(v3.0 既有,直连拓宽窗口,2026-08-10 复盘补录):引擎 tick"开头整批加载状态、逐渠道整行 upsert 写回"会覆盖窗口内并发到达的探针结果(agent 上报或直连 goroutine 同理);同站点前序渠道一笔 15s 直连写会把后续渠道的覆盖窗口从毫秒拉到秒级。后果有界:结果被覆盖→10 分钟探针丢失兜底重置重探,仅延迟一轮恢复。正确修法=状态落库对探针字段条件更新(不复活已完成轮次),动共享 SQL,记档进 B2 随探针字段拆分一并做。
