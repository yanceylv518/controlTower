...
- P3(2026-08-10 复盘修正结论):HTTP 写成功但落流水失败→newapi 已变而 CT 记账停留旧值——**对账回路会捕获,但捕获形态是打着"人工修改"错误标签的不可自愈暂停**(解除条件=快照回到旧值,永不满足,需人工处理)。触发窗口极窄:须恰好该笔事务失败而几毫秒后的状态落库成功(DB 整体故障时 tick 更早就中止);失败方向安全侧(停下来喊人)。~~缓解可选~~ **已实施,见 §12**。
- P3(v3.0 既有,直连拓宽窗口,2026-08-10 复盘补录):引擎 tick"开头整批加载状态、逐渠道整行 upsert 写回"会覆盖窗口内并发到达的探针结果(agent 上报或直连 goroutine 同理);同站点前序渠道一笔 15s 直连写会把后续渠道的覆盖窗口从毫秒拉到秒级。后果有界:结果被覆盖→10 分钟探针丢失兜底重置重探,仅延迟一轮恢复。~~记档进 B2~~ **已实施(精确 id 守卫),见 §12**。
- 记档:token 在 newapi 侧轮换后需到实例管理页重新保存直连配置(无自动探测,失败表现即上条 P2);~~直连探针轮完成早于引擎落 ProbeCommandID 的竞态~~——**我原判"亚毫秒理论竞态"是错的(codex 指出,2026-08-10):probe_count=1 或瞬时失败(连接拒绝/401)时整轮毫秒级结束,先于引擎落库,结果 UPDATE 匹配 0 行整轮丢失,恰发生在探针最需要工作的故障场景。已修:探针轮开跑前先等状态行出现本轮 command id(200ms 轮询,30s 上限,等不到则命令置 failed 交由兜底),顺序钉死而非赌时延;三个单测+真库集成复跑通过**。
...
## 12. 两条 P3 一并闭环(2026-08-10 用户发令)

1. **写成功落流水失败不再分叉**:wrapper 落流水失败重试一次,再败**按"已写入"返回**并大声记日志——写入确实到达了 newapi,谎报失败才是 manual_override 误标+错误计数的根源;状态跟随现实,缺的只是那一笔纸面记录。单测两例(瞬时失败重试即成/两败仍报已写)。
2. **探针结果防复活守卫**:RecordContinuousProbeResult 把完成轮 id 记入新列 last_probe_command_id(043 增量;引擎整行 upsert 永不触碰此列),PutContinuousState 对探针四字段条件更新——**只拒绝"把已完成的那轮 id 写回"**。第一版守卫(按"行=NULL+attempts>0"判)被集成测试证伪:该行形状与"引擎内存折叠完毕同笔落库开新轮"的合法路径无法区分,精确 id 比对零歧义。agent 上报路径同函数同守卫。真库断言:陈旧整行写不复活已完成轮/引擎合法重置照常通过。

§8 对应两条 P3 就此闭环;直连系列剩余记档仅"重启遗留 delivered 残迹"(纯 DB 痕迹,兜底完备)。
