...
## 现场记录（2026-08-07 用户报告后自愈）

用户报告"导出账单失败 + 日账单无数据"，随后自行恢复。事后推断最可能原因：v6 全量重算 + 扫描限速使生成变慢，**任务运行期间**新版本未激活——日账单空、工作簿 409 被折叠为"导出失败"，任务完成后自愈。属生成期正常空窗但产品表现误导（P3"409 原因折叠"实际咬人一次，升级为待修：任务运行中导出/日账单应显示"生成中 N%"而非失败）。另一不可排除的可能：故障时跑的是 rc33 强制预检版。**排障口诀入档：账单页任何"没数据/导出失败"先看顶部任务状态条**。
