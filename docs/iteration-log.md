...
- `933761b2` 已推送 main：归档基础与统一逐日任务、旧月表接入、源读取证据/目标核验/封存、页面和自动金额快照。
- 独立暂存树 Go vet/test 通过；本轮先前 Web/实库验证见归档交接。未打版本标签、发布或部署；目标库需 prepare 到022，CT迁移到088。
...
## v2.0.0-rc126（2026-09-20）

### 版本定位与迭代思路
交付测试账户/Key调用跟进及电话提醒，同时包含渠道亲和性日志展示。

### 功能范围
测试名单、调用检测、专用运营人员、电话/群提醒、按站点语音模板及显示名称、提醒记录和旧后端界面预览；客户流量继续直接配置接听号码。包含084数据库迁移。

### 开发过程与关键决策
标签指向ca5fe985，包含渠道亲和性提交22a43b2c。远程CI 35509878945成功；本地同步后Go vet/test、typecheck/build及277项前端回归通过。未提交的日志归档和账单工作不在包内。

### 开发问题
旧后端返回404导致页面空白，补充明确标注示例数据的只读预览，所有预览写入被禁用；运营人员限定测试提醒使用。

### 部署实录
用户明确要求远程打包，推送v2.0.0-rc126触发[release 35510215788](https://github.com/yanceylv518/controlTower/actions/runs/35510215788)。构建、GHCR版本/latest镜像推送、Release创建成功；[Release](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc126)为非草稿，Server linux-amd64、Agent linux-amd64/arm64及SHA256SUMS四附件均uploaded。未部署服务器。

### 部署问题
无，尚未安装到目标服务器。

### 已知限制
未下载附件做本地哈希复核或Linux实机安装；真实日志读取、模板和接听待验收。检测连接需可查询logs/users/tokens，增量回看边界见测试跟进任务记录。

### 遗留问题
当前关闭操作仍为“暂停”；“关闭测试”命名及重新开启自动新一轮尚未实现。未进行真实外呼。

### 下一版本方向
确认测试对象生命周期，完成真实站点及电话联调。

## v2.0.0-rc125（2026-09-20）

### 版本定位与迭代思路
补齐模型广场价格展示，并修复手机日志时间范围。

### 功能范围
币种遵循站点配置、表达式档位单价、紧凑模型卡片及名称复制；日志日期和时分秒分别编辑与完整展示。

### 开发过程与关键决策
标签指向6739e0fa，包含7b1aab77。252项前端回归、typecheck/build通过。表达式仅提取可确定价格，不执行上游表达式。

### 开发问题
表达式列表撑高整行卡片，改为统一价格摘要，完整规则置于详情。

### 部署实录
CI 35478573343及release 35478586019成功；[Release](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc125)已上传Server amd64、Agent双架构及SHA256SUMS。GHCR版本及latest镜像发布成功，未部署服务器。

### 部署问题
无，尚未部署。

### 已知限制
未Linux实机安装，未下载附件作本地哈希复核。最终卡片手机浏览器检查通过，桌面最终布局与复制操作未实测；任意复杂公式保留原文。
...
## v2.0.0-rc124（2026-09-20）

### 版本定位与迭代思路
提供管理员与查看账号手机布局，同时保留桌面工作流。

### 功能范围
手机导航、日志和用户列表滚动加载、日志错误码、调权优先级/分组与保存预览，以及概览和监控控件密度优化。

### 开发过程与关键决策
手机业务278f0251衔接最新主线日志优化2a38432d；版本标签指向3a7f57bb。247项前端测试、类型检查、构建通过，主线合并后Go vet/test通过。

### 开发问题
合并新版桌面样式后出现手机模型区被撑高及按钮文字偏上，已通过小屏规则修正并实测。

### 部署实录
用户确认正式发布，GitHub Actions release 35476816881成功；[Release](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc124)包含Server amd64、Agent双架构与SHA256SUMS；GHCR版本镜像和latest发布成功。未执行服务器部署。

### 部署问题
无；尚未执行部署。

### 已知限制
未完成新版全站真机视觉回归和Linux实机安装。正式包由Linux CI重建，与先前Windows候选包哈希不同，应使用对应附件校验和。
...
## v2.0.0-rc121 调权、日志链路与账单页面整理（已发布，2026-09-18）

### 版本定位与迭代思路

交付 rc120 后已整合的业务与界面改进。

### 功能范围

平均输出和调权、只读渠道来源、账单任务选项、动态计费摘要、站点分组组合、fallback 请求链路及全站和上游账单布局。

### 开发过程与关键决策

标签指向 e99e291f，保持既有标签打包流程；包含 Server/Agent/Web 和 081、082 迁移。

### 开发问题

本次无代码调整。169 项前端回归通过；对应提交 CI 的 Go 与 Web 质量门通过。

### 部署实录

仅发布未部署。CI 35292285605、release 35292548665 成功；三份安装包下载并通过 SHA-256，前端及迁移文件已核验，GHCR rc121/latest 已发布。[发布页](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc121)。

### 部署问题

无，未部署。升级需先更新 Server/前端及迁移，再更新 Agent。

### 已知限制

真实浏览器及生产升级未验收；旧 Server 无分组组合库接口，须升级。
...
## v2.0.0-rc120 账单列序及用量缺价提示（已发布，2026-09-16）

### 版本定位与迭代思路

按照用量、价格、费用阅读账单，避免把无用量当成单价缺失。

### 功能范围

主账单四表最终费用末列；图像及缓存可选单价结合实际用量显示。

### 开发过程与关键决策

提交 e9586e32 推送 main；表头、行值、列宽与折扣跨表引用同步调整。保留旧规则证据，不扫描全部明细。

### 开发问题

新增列追加到金额之后、缺字段与无用量混淆；通过列重排及明确用量状态修正。全量 Go vet/test 和生成 XLSX 回归通过，CI Go/Web 通过。

### 部署实录

仅发布未部署。CI 35035653678、release 35035657962 成功；三份安装包下载并通过 SHA-256，GHCR rc120/latest 已发布。[发布页](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc120)。
...
旧规则未记录用量状态，只给中性说明。新生成记录才能准确区分；无真实 Excel/生产验收。

### 遗留问题

逐笔公式核对待实现；表达式仍显示见计价规则。

### 下一版本方向

按用户安排部署和验收，仅需升级 Server，无新增迁移。旧账单重新下载主账单可更新列序。

---

## v2.0.0-rc119 价格规则缺失信息展示修复（已发布，2026-09-16）
...
同条件且已知价格相容时合并展示，并说明缺失字段；真正变价、零价、不同表达式及条件仍区分。

### 开发过程与关键决策

修复 e42ae728 已推送 main，rc119 指向该提交。仅修改展示，不改金额或原始证据，不扫描订单明细。

### 开发问题

整段规则文本去重将图像价格20与未记录算成两套；改为识别相容已知价格。新增回归覆盖截图与缺失记录桥接冲突等边界，Go 全量 vet/test 通过。

### 部署实录

仅发布未部署。CI 35033102853、release 35033106999 成功；三份包 SHA-256 匹配。GHCR rc119/latest 已更新。[发布页](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc119)。
...
按用户安排验收及推进公式核对。只需升级 Server，无新迁移，rc118 账单无需重生成。

---

## v2.0.0-rc118 历史单价与日计价规则（已发布，2026-09-16）

### 版本定位与迭代思路

补充原始计费账单的历史价格可解释性，金额仍采用日志扣费。

### 功能范围

明细历史单价、表达式/档位/规则；日统计和导出逐套展示同日不同价格，小型规则工作表避免扫描逐订单明细。

### 开发过程与关键决策

ba9a2f5e 推送 main，标签 v2.0.0-rc118 指向该提交；新任务 usage_version=2，旧文件不回填。用户授权提交、推送、重新打包。

### 开发问题

无新增阻塞；既有精度和旧音频任务兼容审查事项保留。

### 部署实录

仅发布，未部署。CI 35031183849、release 35031229720 成功；三份安装包 SHA-256 匹配。镜像 ghcr.io/yanceylv518/controltower-server:v2.0.0-rc118 及 latest 已更新。[发布页](https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc118)。
...
旧账单需新建任务获得价格信息；表达式不保证拆成固定单价；音频分项单价暂不独立还原。无新迁移，依赖已有 080。

### 遗留问题

真实账单、浏览器及 Excel 人工验收待执行，百万级性能未压测。

### 下一版本方向

按用户安排部署验收。完整记录见 [任务交接](ai-tasks/2026-09-16-billing-historical-prices.md)。

---

## v2.7-B3 稀疏指标曲线修正（完成，2026-07-16）

TTFT 与缓存命中率曲线已启用稀疏序列渲染，孤立点和跨空分钟走势不再不可见；同一分钟的部分聚合不再清空 TTFT P95，1m 合并与 5m 汇总均使用非空 P95 最大值作为保守上界，NULL 语义保持不变。缓存命中计数的 5m 可加汇总已通过测试核对。完整说明见 `docs/v2.7-b3-delivery.md`。
...
## 2026-07-17：监控详情页维度快速切换

- 客户、渠道、模型详情页的当前维度标题改为可搜索下拉框，选项沿用当前实例的同维度列表。
- 选择其他条目后直接切换对应详情，保留返回列表、上一个/下一个和时间范围操作。
