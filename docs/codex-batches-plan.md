...
| M1-B1 | Server 认证体系（users/sessions/登录锁定） | M0-lite | 浏览器手测登录/登出/改密/锁定，~20 分钟 |
| M1-B2 | 实例管理 + 按实例 Agent Token + 多实例过滤 | M1-B1 | 建实例、轮换 token，~30 分钟 |
| M1-B3 | 告警事件时间线 + 通知强化 | M1-B1 | Web 走告警生命周期 + 重发死信，~20 分钟 |
| M1-B4 | 渠道命令闭环 + API 硬化 + 契约冻结 | M1-B2 | Web 停用/调权测试渠道核对审计，~20 分钟 |
