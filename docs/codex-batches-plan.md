...
>
> **2026-07-13 前端依赖已批准**（Node≥20+pnpm、Vue3/Vite/TS/Router/Pinia、Element Plus、ECharts、Vant4），M2 启动。
>
...
| M2-B1 | 行走骨架：pnpm 工作区 + shared API client + 登录页 + 总览页 + Go 托管 /next/ + CI 前端质量门 | M1 契约冻结（已达成） | 并入 M2 阶段点 |
| M2-B2 | 通用组件沉淀 + 只读页（客户/渠道/模型监控、样本分析、系统状态、用量统计） | M2-B1 | 并入 M2 阶段点 |
| M2-B3 | 操作页（告警中心+时间线、通知设置+重发、实例管理+token、渠道命令下发） | M2-B2 | 并入 M2 阶段点 |
| M2-B4 | 设置页 + 自动刷新/暗色等横切打磨 + `/next` 切换为 `/` 并删除旧静态页 | M2-B3 | **M2 阶段点：浏览器过全部页面（~1 小时）** |
