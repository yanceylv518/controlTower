...
> **2026-07-13 方向修正（用户决策）**：主线是监控系统产品（Server → Web → App）。告警 Agent 线暂停深化——生产继续跑 v1.0.6，**B1 已合入主干但不单独部署**（下次 Agent 升级时随车带上），B2/B3 挂起（设计保留，需要升级钉钉告警时再启动），验证机暂不搭建。执行顺序改为：**M0-lite（CI）→ M1 Server 四批次 → M2 Web → v2.0 发布（Agent 届时一次性升级接入双模式，B1 功能与渠道快照常驻化修复随该次升级上线）**。
