...
- **P2 记档待办（用户拍板 2026-08-07：下次再修，暂不影响）**：请求明细页对未配 CT 价的模型 `SelectPrice !ok → continue` 静默跳行——整页可能为空。**修法方向已由用户定调：基座价不应依赖 CT 配置初始值，应从 newapi 加载**（ratio 快照现算回退价作基座，CT 配价仅为覆盖项）——如此该缺口自然消失，且与"行内倍率优先/以 newapi 实际为准"方向一致。待立项时按此实现；
