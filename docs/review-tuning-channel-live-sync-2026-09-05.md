...
## 追加：取消编辑会回退已同步的线上状态显示（2026-09-06，用户指出，P2，已修）

- **现象**：页面处于未保存编辑（脏）状态时，30 秒轮询或变更通知合并进来的线上
  列（当前权重/线上优先级/快照时间等）只写入 `bases`，`savedBases` 不更新；点
  "取消更改"用 `savedBases` 整体覆盖 `bases`，线上列被一并回退到上次加载时的旧值，
  直到下一次轮询才恢复。
- **修正**：`mergeOnlineRows` 先按服务端最新行合成 `merged`（含晚到旧请求的快照时间
  保护），`savedBases` **始终**取 `merged` 的副本——取消未保存编辑的语义是回到
  服务端当前已保存状态（含他人保存的基础值），而非回到本地上次加载；`bases` 仅在
  脏时保留本地四个可编辑字段。
- 实证（无头浏览器 CDP）：把渠道 1 基础权重改为 123 进入脏状态 → 后台把线上权重
  改为 77 → 30 秒轮询合并后当前权重显示 77 → 点"取消更改"→ 当前权重仍为 77，
  基础权重回到已保存的 100。`pnpm typecheck`/`build` 通过。
- 部署：只改前端，随本批进 rc96。

## 追加：af37a84 首屏遮罩先释放、刷新后选中新模型（2026-09-06 复验通过，零缺陷，codex 分支快进合入）

- 修的是 09-05 P3 修复批里我做得不完整的两处：①`load(true)` 虽然把直连刷新
  与数据加载并行，但 `await synced` 仍在 loading 的 try 块内，遮罩要等直连返回
  才消失，"首屏不等直连"只做对了数据、没做对遮罩；②直连刷新带来新模型时
  `mergeOnlineRows` 没补 `dispatch_modes` 默认值、也没选中，新模型不可见。
  另加 `isCurrentLoad` 代际守卫与卸载时递增代际，导航后的迟到响应一律丢弃。
- 复验：`pnpm typecheck`/`build` 通过。CDP 实证：把 demo 直连地址指向黑洞
  `10.255.255.1`，打开调权中心 1.1 秒遮罩释放且三条渠道已渲染，此时直连刷新
  仍在等待（上限 45 秒）。
- 只改前端，随本批进 rc96。
