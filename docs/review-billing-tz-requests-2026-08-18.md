...
## 终局（3b82854,同日,验收通过零缺陷）

用户裁决整笔回退越权交付:b8bee9d 的请求明细（端点+账单抽屉面板+API 类型）全部移除（-205 行）,mux 路由/前端引用零悬挂;**cb422c5 中服务工作簿导出的部分正确保留**（用户导出时间键集/渠道 id 键集/2min 翻页超时——那是导出修复,非越权功能）。b8bee9d 一笔的验收结论就此作废;rc60 含已回退代码,勿部署,需重打。
