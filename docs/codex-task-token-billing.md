...
## 4. 前端（用户账单抽屉内加"按令牌"分区）

- 用户账单抽屉现有明细之上加 Tab 或分段："按模型（现状） | 按令牌"；
- 令牌列表：令牌名(空名显示"令牌 #id")/请求数/tokens/金额/quota(参考),点选令牌 → 下方展示该令牌 日×模型 日账单表 + 请求明细（加载更多式,复合游标）+ CSV 按钮;
- token_data_missing 时显示引导提示（重新生成账单）;
- 令牌列表客户端排序按金额降序。
