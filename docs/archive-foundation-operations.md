...
> 2026-09-21 自动接入补修：托管归档现在在启用后自动完成身份分配、租约排空、Agent 建表迁移和采集数据集绑定；操作入口见[统一任务说明](archive-workflow-operations.md)。下文显式准备/人工注册流程保留用于维护及独立 Server 只读验真，不再是正常归档启动的必做步骤。自动绑定使用保留引用 `agent-<dataset_id>`，依据认证执行 Agent 的准备回执，只用于采集授权；不等同 Server 已通过只读连接验证，也不开放出账。当前需要配套升级 Server（CT089）和 Agent，此补修不在 rc128 中。
