...
## 4. 前端：渠道账单页新增"上游渠道"视角

- 渠道账单页（ChannelBillingV4）加 Tab 或切换器："按渠道 | 按上游 key"；
- 上游列表：显示名/成员数/请求数/tokens 四列/quota（**列序按对账主次:请求数与 tokens 前置,quota 标"参考"**）;
- 点开组 → 抽屉或下钻：日×模型明细表+成员渠道小计+CSV 导出按钮;
- 未归组桶显示"未归组（快照缺失）"并列出 channel_id。
