...
## 终版设计（1200d99,验收通过——取代 41869c6 的调整方式）

41869c6 把计费额本身切换成旧版负通道公式,使计费口径随日志年代分叉。
1200d99 收敛:**计费额统一按现版（newapi rc23 钳位）公式**（样例计
179406）;校验时若旧版公式（负通道调整后）恰好解释实扣差额（容差 1 内
精确匹配,非放宽容差）,该行 Verified 进正常账单,Difference 仍如实记录
（样例 20）。效果:现版钳位行直接吻合、旧版行差额被精确解释,两类溢出行
都归正常单——41869c6 记档的「现版行反向 mismatch」取舍随之消除。旧版行
账单额比实扣高出 |溢出|×倍率（几个 Token 量级,样例 ¥0.00004）,属口径
统一的已知偏差。守卫不变（token 模式/非 anthropic/非多模态/普通输入为负）。

## 测试

- server 树 vet + test 全绿（含 CT_MYSQL_TEST_DSN 真库）;回归改双断言
  （现版 179406 直接吻合+旧版 179386 差 20 被解释放行）;webapp
  typecheck + build 通过（41869c6 轮已核,1200d99 未触前端）;
  agent/internal 未触及。
