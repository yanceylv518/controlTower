...
## 跟进二（41a0c65+bbe94a1,2026-08-18,验收通过零缺陷——用户导出第三版:时间窗转 id 窗）

- **41a0c65**:工作簿明细页大小从 BillingPageSize 独立为 500 常量（每页带 JSON 投影+过滤,防单页独占只读连接或顶到 2min 上限）,测试钉 500;
- **bbe94a1**:用户导出改"时间窗转 id 窗"——两条标量子查询按 idx_created_at_id 把 from/to 换算成 id 边界,主扫描 FORCE INDEX(idx_user_id_id) 在边界内按 (user_id,id) 键集翻页。**两索引名对 newapi 源码逐字实证均原生实存**（model/log.go gorm 标签）;7 占位符顺序与 args 拼装逐一核对一致且测试钉住;created_at 范围谓词仍保留在主 WHERE。此即 8 月 5 日选路设计定稿（design-readonly-logs-query-routing）的核心手法首次落到导出路径——三版演进:纯 id 走查（被生产证伪）→时间键集（大用户仍慢）→窗转 id+用户索引（两索引各干各的活）。

P3 记档:id 窗边界继承窗转手法的固有近似——created_at 在窗内但 id 因提交乱序落在边界 id 之外的行会被漏（秒级偏差、仅月边界附近、与选路设计已知取舍同源）;账单生成路径不受影响（无筛选任务仍走纯时间键集）。部署后 EXPLAIN 清单更新:单用户导出验证两级索引各就位（子查询 idx_created_at_id/主扫描 idx_user_id_id）。
