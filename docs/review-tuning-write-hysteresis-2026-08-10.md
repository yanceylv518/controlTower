...
observe 事件死区改为**水平过滤**:状态新增 last_observed_weight 锚点（044 增量,引擎记录事件时同步推进锚点),死区比较对象从"上一 tick 计算值"改为"上次已记录事件值"——缓慢漂移在累计越过死区时恰记录一次（专项测试:2 单位/tick 漂移三轮,恰一条事件）;既有 observe 测试改为操纵锚点,断言记录后锚点推进。全量+真库集成（含 044 双重放）绿。含本批与本跟进需 rc44,迁移 033-044。
