...
## 收官验收（2026-08-02 追加，bb3a07f）：**v3.1-B3 整批完成**

**计价配置界面**交付：价格/分组倍率双 Tab、阶梯编辑（加档/生效日期）、"从 newapi 导入"建议列表（过滤已配模型）、admin-only 三重守卫（路由 adminOnly + viewer 重定向 + 服务端闸不放行）。typecheck + build 绿。

**B3 全批盘点**：五轮分批交付（地基→日切与回填→API→闸/缓存/导入/账单页→计价界面），批次文件全部条目落地；验收方补齐三处（登录审计属 B1 遗留、CSV BOM、api-contracts）并修复实例表回归与直连页白屏。**整批自查清单始终未贴**（分批交付模式下的流程缺口，最终记档）。

## 用户侧收尾（步骤 6）

配价（或先导入再改）→ 确认 newapi 日志保留边界 → 触发历史回填 → 抽两用户与 newapi 控制台对账（quota 对照列偏差应在舍入误差内）→ 验收通过后打 rc24，v3.1 全系列交付完成。
