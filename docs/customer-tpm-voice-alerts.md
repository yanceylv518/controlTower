...
- 受系统告警通知总开关 `CT_NOTIFICATIONS_ENABLED` 控制；`CT_NOTIFY_BALANCE_ONLY` 只控制原 Webhook 路径，不屏蔽显式开启的 TPM 电话预警。
- API-only 模式不运行电话后台。配置页显示该状态。主进程与其他 worker 一起停止。

## 部署和启用

1. 先部署 Server，执行 `066_user_tpm_voice.sql`；再升级所有日志采集 Agent，最后更新 Web。不要先把新 Agent 接入不支持客户秒桶的旧 Server。
...
`GET/PUT /api/dashboard/voice-alerts` 仅管理员 Session 或已有管理 Bearer Token 可用，沿用 CSRF 校验。密钥不会返回到页面。电话记录中的手机号脱敏；配置修改写入操作审计，审计不保留明文手机号。
