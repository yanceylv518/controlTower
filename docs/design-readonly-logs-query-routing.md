...
**边界取法修正（2026-08-05 复审回应，替代上面的 MIN/MAX 写法）**：上述问题 1 成立——聚合写法在 (created_at, id) 索引上是范围扫不是取端点，`MAX(id) WHERE created_at<=end` 甚至要扫全部历史，不可用。改为**索引定位 + 松弛边界**：

```sql
-- 各一次 O(log n) 索引寻位，毫秒级（与聚合 runner 已在生产运行的同型查询）
SELECT id FROM logs WHERE created_at >= ?  /* start-Δ */ ORDER BY created_at ASC,  id ASC  LIMIT 1;  -- lo
SELECT id FROM logs WHERE created_at <  ?  /* end+Δ   */ ORDER BY created_at DESC, id DESC LIMIT 1;  -- hi
```

Δ 取 60s：id/时间的局部乱序来自并发提交，量级是亚秒到秒（logs 为逐请求 autocommit 写入），60s 松弛带足以覆盖；乱序行只会落在松弛带内，不会被 id 窗夹掉。配合下述"列表 SQL 保留精确 created_at 条件"，**结果正确性完全不依赖同序假设**——边界只负责裁剪候选集，取松了无害（多扫一点），等价性由 created_at 条件兜底。这同时回应问题 2：不需要 newapi 提供任何单调性约束。补写历史日志（created_at 远早于 id 序）是唯一超出松弛带的场景——newapi 无此行为，若将来出现则该行在旧时间窗查询中不可见，记档为已知边界。

利用 InnoDB 二级索引携带的主键后缀，让等值条件走对应索引，再用 id 窗缩小候选范围。典型改写（时间+模型+类型）保留真实时间条件：
...
1. 生产数据上的 `MIN(id)` / `MAX(id)` 边界查询实际扫描多少行、耗时多少？是否会把原来的一条慢查询变成两条边界慢查询加一条列表查询？
   **→ 设计层已闭环（2026-08-05）**：评审意见成立，聚合写法废弃；改为索引定位 + Δ=60s 松弛边界（见 §3 修正块），两次 O(log n) 寻位，与聚合 runner 生产在跑的同型查询。待生产 EXPLAIN 例行确认，无未知风险。
2. newapi 是否对 `id` 与 `created_at` 的严格单调关系有可依赖的约束？如果没有，如何证明 id 窗不会漏掉并发乱序或补写数据？无论答案如何，最终列表 SQL 都必须保留 `created_at` 精确条件。
   **→ 设计层已闭环（2026-08-05）**：不依赖任何单调性约束——列表 SQL 保留精确 created_at 条件（采纳），id 窗仅裁剪候选；松弛边界保证乱序行不被夹掉（并发乱序量级亚秒到秒，Δ=60s 覆盖）。唯一例外"补写历史"newapi 无此行为，记档为已知边界（见 §3 修正块）。
3. 各站点的索引名称、列顺序、MySQL 版本及 `use_index_extensions` 是否一致？索引能力探测和安全回落由哪一层负责？
   **→ 方向已定，细节留待批次**：CT server 启动/站点配置变更时 `SHOW INDEX` 探测一次并缓存（复用只读连接），选路表只路由到实际存在的索引，缺失或探测失败一律回落阶段 A 基线路径——FORCE INDEX 永不指向未经探测确认的索引名。`use_index_extensions` 与版本差异归入生产 EXPLAIN 验证（§7.1）。
4. 产品是否接受从“可点击任意页码”改为“仅上一页/下一页”的游标交互？如果不接受，游标分页不进入首版。
   **→ 待用户拍板**。备选折中：页码跳转保留 OFFSET（跳页本来就少且通常跳得浅），顺序上一页/下一页走游标——两者可共存，代价是接口同时支持两种翻页参数。
5. `other` 与 `content` 的首屏字段依赖如何拆分？详情接口返回什么、何时请求、失败时如何降级？
   **→ 留待批次前梳理**：评审意见成立（TTFT/缓存 token/流式标签等为列表行内展示，源自 `other`，不是展开才用）；写批次时先出字段级依赖清单，再定"服务端解析成窄字段随列表返回 vs 详情懒加载"的分界。首版可仅做 §3 的 SQL 选路不动返回字段——两个改动正交，不必绑定。

建议分两阶段推进：

- **阶段 A（低风险基线）**：仅将时间条件改为半开区间，排序改为 `created_at DESC,id DESC`，显式验证 `idx_created_at_id`，保留现有页码和返回字段；先取得生产耗时基线。**→ 已实施（012235f，验收 review-anomaly-input-time-index-2026-08-06.md）**：SQL 形态有契约测试；未加 FORCE INDEX，生产 EXPLAIN 实证（§7.1 路径②）待部署后完成。
