...
window.DEVLOG = [
  {
    ...
    summary: "验收 be192b8 时记档的两个 P3 清账：①客户/渠道/模型监控页的『超阈值/接近阈值』状态签与 KPI 计数不再硬编码 2 秒/1.5 秒，改为对照配置的 P95 阈值（CT_TTFT_P95_THRESHOLD_SECONDS，默认 60 秒），75% 起算『接近阈值』，KPI 小字动态显示当前阈值，与图表虚线同源不再互相矛盾；②设置页三个阈值输入框下限 0.5 对齐服务端校验的 0.1。注意行为变化：默认配置下状态签比原 2 秒标准宽松得多，趋近 B2『≥60s 超量程』语义；若需要更严的健康标准，在设置页调低 P95 阈值即可。typecheck 与生产构建绿。",
    docs: [],
    commits: []
  },
  {
  ...
  },
  {
    ...
    type: "bugfix",
    version: "",
    title: "总览突出全站 TPM 与成功率趋势",
    summary: "总览页将 TPM 提升为首要 KPI，并把原来的请求量/错误数单图改为独立的 TPM 与成功率近一小时曲线。两张图复用当前实例已有的 1 分钟历史指标，不增加接口或数据库查询，也不降低聚合频率；成功率固定使用 0%—100% 纵轴，便于快速识别吞吐变化和可用性波动。",
    docs: [],
    commits: []
  },
  {
  ...
  },
  {
    ...
    summary: "修复客户、渠道和模型详情页顶部区间请求数与下方关联维度列表不一致：顶部原本按所选 1h/6h 等区间汇总，而列表只读取每个关联维度最新一个指标桶。新增按维度键前缀查询历史指标并逐关联维度分组汇总的接口能力，详情列表改为使用与顶部完全相同的时间窗口；补充接口分组隔离、MySQL 查询结构回归测试，并通过 Go 测试、前端类型检查与生产构建。",
    docs: [],
    commits: []
  },
  {
  ...
  },
  {
    ...
    summary: "健康检查从运行状态按天保留策略中拆分，新增 CT_RETENTION_HEALTH_HOURS，默认 6 小时且可在设置页调整。Server 指标和容器状态继续使用原运行状态天数；保留清理从每日改为每小时执行，确保高频健康检查数据及时回收。",
    docs: [],
    commits: []
  },
  {
    ...
    version: "",
    title: "系统状态健康检查信息降噪",
    summary: "系统状态页的健康检查主表改为按实例和检查目标仅保留最近一次结果，异常状态优先排列，并在标题区汇总正常与异常目标数量。最多 200 条原始检查历史不再直接铺满页面，改为默认收起的限高排障表。",
    docs: [],
    commits: []
  },
  {
    ...
    version: "",
    title: "设置页宽屏双列布局优化",
    summary: "移除设置页 980px 固定宽度限制，将数据保留和告警阈值放入主配置列，通知、默认实例、显示与账户配置放入辅助列。宽屏充分利用右侧空间并减少无意义的纵向滚动，窄屏自动回退为单列布局。",
    docs: [],
    commits: []
  },
  {
    ...
    version: "",
    title: "监控页面支持配置默认实例",
    summary: "系统设置新增默认实例选择，登录或刷新后总览、客户、渠道、模型、告警、样本、运行状态、审计与用量统计等页面优先加载该实例，避免多实例数据默认混排；顶部选择器仍支持临时切换其他实例或全部实例。默认实例被禁用或删除时自动回退到第一个可用实例，用量统计接口同时补齐 instance_id 过滤。",
    docs: [],
    commits: []
  },
  {
    ...
    type: "bugfix",
    version: "",
    title: "维度列表列宽分配与右侧操作防溢出",
    summary: "修复客户、渠道、模型监控表格仅名称列吸收全部剩余宽度，导致左侧大面积空白而右侧详情/操作仍被截断的问题。指标列由固定宽度改为最小宽度并共同分配可用空间，名称列设置合理基线，Token 列适度扩宽；最右操作列扩容并固定在右侧，窄屏出现横向滚动时仍可操作。",
    docs: [],
    commits: []
  },
  {
    ...
    type: "bugfix",
    version: "",
    title: "渠道列表密度与操作区留白调整",
    summary: "根据实际宽屏列表反馈，渠道监控表格行距过密、操作与详情入口紧贴。统一提高表头和数据行纵向留白，适度增大正文字号与单元格横向内边距；操作列扩宽并为操作按钮、详情入口增加明确间距，在保持大列表可扫描性的同时减少拥挤感和误触。",
    docs: [],
    commits: []
  },
  {
    ...
    version: "v2.0.0-rc10",
    title: "rc10 发布",
    summary: "自 rc9 后：告警保留期设置(所有告警按最后出现时间超期清理,默认30天)+告警中心一键清理(已确认/已静默/已解决/全部非活动)+默认仅显示活动告警;修复 defaultAlertSettings 手抄默认值导致阈值归零的真 bug(告警刷屏的第二元凶);样本分析↔延时分诊按 request_id 双向关联(样本详情抽屉对齐业务账与网关账);延时分诊页内使用说明;渠道操作移入渠道列表行内抽屉。仅需升级 Server,Agent 无改动。",
    docs: [],
    commits: []
  },
  {
