...
- P3 记档：计数查询失败会 500 整个 summary/detail 响应——异常计数属附属信息，可降级为不显示而非阻塞主表，后续顺手改。**澄清（用户问询后补）**：这是故障耦合记档，不是性能问题——查询打 CT 自库小表、summary 侧还有 job 级缓存，正常运行零感知；
- P3 记档：billing_anomaly_orders 无 job_id 索引，计数查询为全表扫——表只存异常单目前量级极小无感；若将来积累到几十万行，补 job_id 索引（顺手迁移即可）。
