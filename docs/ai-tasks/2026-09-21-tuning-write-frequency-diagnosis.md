...
- 状态：CT 暂停后无法重试及观察权重读回缺陷已修复并完成独立交付验证，按用户授权提交推送中，未发布/部署；reset续查已收敛至CT到ALB的特定连接/链路方向，准确RST成因仍缺连接级证据。
...
- 下一步：按用户后续发布/部署指令交付暂停恢复修复；若继续查reset，补CT httptrace连接信息并在故障复现时做CT侧抓包/两ALB入口对照，必要时用准确时间和五元组向云厂商查RST来源。ALB访问日志需另行评估开通（控制台提示SLS按量计费），本轮未开启。
