...
## 缺失信息误报多套价格修复（2026-09-16，rc119 已发布）
...
- 用户授权提交推送打包。fetch 后 main 与 HEAD 均为42edaa0b，rc119 标签不存在；六个修复文件提交 e42ae72869c220d5350e369ecc47c48ac124a0d9 并推送 main，标签 v2.0.0-rc119 指向该提交。未改动主检出的用户文件。
- CI 35033102853、release 35033106999 均 completed/success；Go/Web 质量门、安装包、镜像推送、Release 创建全部成功。
- 发布页：https://github.com/yanceylv518/controlTower/releases/tag/v2.0.0-rc119；镜像 ghcr.io/yanceylv518/controltower-server:v2.0.0-rc119，latest 同步更新。
- 三份安装包与 SHA256SUMS 已下载至主目录 release/v2.0.0-rc119，三个 SHA-256 均匹配；Server 包内二进制、前端入口、080 迁移已核对。
- 无新增迁移，本修复只需升级 Server；现有 rc118 账单无需重生成。未部署、未做生产验收；逐订单独立公式核对尚未实现，不包含在本次包内。
