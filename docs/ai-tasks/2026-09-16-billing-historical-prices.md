...
- 目标：用量、单价和规则在费用之前，最终费用为末列。
- 范围：隔离分支 codex/billing-order-verification；仅 Server 主账单 Excel 输出和回归测试，主工作区其他改动保留。
- 结果：账单总览、每日账单、令牌用量汇总、令牌每日用量统一调整多媒体用量顺序；每日账单价格/规则在总费用前，明细文件移到标识列区域。用户/上游输出共用流程；页面已是用量、价格、费用顺序，无需前端修改。
- 公式：总览折扣列随多媒体列移动，每日表引用同步调整；表头、列宽、行数据、合计同步。
- 验证：本次 go vet ./...、go test ./... 通过。生成 XLSX XML 回归检查四表最终费用末列、图像用量值、每日价格/规则顺序和折扣引用；历史价格测试继续验证渠道过滤。未运行真实 Excel/浏览器/MySQL 验收；无 Web 修改。
- 交付：未提交、未打包、未部署；无迁移，升级 Server 后重新下载主账单，已有每日明细文件保持原样，不需重新生成任务。逐笔公式核对仍未实现。
- 归档：保留本地记录，以上传结果为准。


## 2026-09-16 无用量与单价缺失区分

- 状态：实现及自动化验证完成，未提交/打包/部署；与列顺序补修一起留在隔离工作区。
- 新生成记录：缺少图像、缓存读取、普通/5m/1h 写入单价时按对应用量分别记录“不适用（无用量）”或“有用量但未记录”。已有数值单价（含零价）保留；音频原有提示仅在音频用量大于零时出现，表达式仍保留原规则。
- 页面和主账单：无用量不参与缺价警告或价格冲突；同一已知单价可以和无用量记录合并，日单价列有数值时隐藏不适用项。实际使用但缺价明确提示。
- 旧账单：旧预聚合仅保存规则，无法证明缺字段订单是否有用量。显示“历史记录未区分无用量与单价缺失”，不再断言部分订单缺价；不扫描全部明细，不改变费用。完整逐项状态仅适用于新生成的文件；已有文件未重写。
- 验证：go vet ./...、go test ./...、git diff --check 通过；覆盖无用量/有用量缺价、已知零价、真实价格变化、旧规则兼容及金额不变。前轮导出列序测试本轮仍通过；无 Web 修改，真实 Excel/生产/MySQL 未验收。
- 无数据库迁移；AI Workspace 上传若不可用则保留此本地记录。


## rc120 发布结果（2026-09-16）

- 用户授权提交推送打包；e9586e32 已推送 main，v2.0.0-rc120 指向该提交。
- CI 35035653678 两项质量门成功；release 35035657962 成功，GHCR rc120/latest 已发布。
- 三份安装包和 SHA256SUMS 已下载至主检出 release/v2.0.0-rc120；三份安装包 SHA-256 全部匹配。
- 仅发布未部署。升级 Server，无新迁移。旧账单重新下载主账单即可更新列序与中性文案；逐项无用量/缺价标记需新生成文件，旧证据不补造。
- 表达式价格仍“见计价规则”；逐笔公式核对未包含。
- AI Workspace 服务此前不可用，归档以实际返回为准；本地交接已保留。
