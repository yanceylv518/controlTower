...
## 跟进三（e16f7f8,2026-08-19,验收通过零缺陷——速度分位权重开放可配）

速度因子的 P50/P90/P95 加权（原硬编码 0.5/0.3/0.2)成为策略参数 speed_p50/p90/p95_weight：默认值=原常量（默认行为严格等价);校验=各自 [0,1] 且**和恒等于 1**（1e-9 容差,拒绝 0.6/0.3/0.2 这类畸形组合,允许 0/0/1 极端配置);旧策略 JSON 兼容读回默认。引擎测试钉配置权重改变因子;悬浮解释与计算权重说明全参数化（w50/w90/w95 记号）;曲线设置面板加三输入。与曲线可配方向一脉相承,交付面单一无越权。
