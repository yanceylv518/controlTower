...
func writeOverview(book *xlsxwriter.Workbook, uid int64, month string, rows []billing.AggregateRow, prices []billing.PriceRecord, ratios []billing.GroupRatio, snapshots map[string]string) error {
	...
	titleRows(s, fmt.Sprintf("%s · %s 对账单", billingUserName(rows), month), month, []string{"序号", "模型名称", "请求数", "输入Token单价/1M", "输入Token", "输入Token总价", "输出Token单价/1M", "输出Token", "输出Token总价", "缓存Token单价/1M", "缓存Token", "缓存Token总价", "账单总金额", "折扣", "账单折扣后总金额"})
	for i, v := range items {
		name := v.ModelName
		if v.TierFrom > 0 {
			name += fmt.Sprintf("（大于等于 %d Token）", v.TierFrom)
		}
		s.Row([]xlsxwriter.Cell{n(i + 1), t(name), n64(v.RequestCount), d(v.InputPrice), n64(v.PromptTokens - v.CacheTokens), d(v.InputAmount), d(v.OutputPrice), n64(v.CompletionTokens), d(v.OutputAmount), d(v.CachePrice), n64(v.CacheTokens), d(v.CacheAmount), d(v.Amount), t(""), t("")})
	}
...
func writeDaily(book *xlsxwriter.Workbook, uid int64, month string, items []billing.DetailItem) error {
	...
	titleRows(s, fmt.Sprintf("%s 日统计", month), month, []string{"日期", "模型名称", "阶梯起点", "请求数", "输入Token", "缓存Token", "输出Token", "输入单价/1M", "缓存单价/1M", "输出单价/1M", "金额"})
	for _, v := range items {
		s.Row([]xlsxwriter.Cell{t(v.Day), t(v.ModelName), n64(v.TierFrom), n64(v.RequestCount), n64(v.PromptTokens - v.CacheTokens), n64(v.CacheTokens), n64(v.CompletionTokens), d(v.InputPrice), d(v.CachePrice), d(v.OutputPrice), d(v.Amount)})
	}
	return nil
}
...
func writeRequestPages(ctx context.Context, book *xlsxwriter.Workbook, month string, from, to time.Time, prices []billing.PriceRecord, ratios []billing.GroupRatio, metadata []billing.ModelMetadata, readPage func(billing.LogCursor, int) ([]billing.PagedLogRecord, error)) error {
	...
	headers := []string{"请求时间", "Request ID", "用户名称", "模型名称", "原始输入 Token", "缓存 Token", "计费输入 Token", "输出 Token", "输入单价/1M", "输出单价/1M", "缓存单价/1M", "输入费用", "输出费用", "缓存费用", "请求总金额", "实际扣除 Quota"}
	sheetNo, rowNo := 1, 0
	...
	for {
		...
		for _, v := range logs {
			...
			billed := v.PromptTokens.Int64 - v.CacheTokens
			if billed < 0 {
				billed = v.PromptTokens.Int64
			}
			s.Row([]xlsxwriter.Cell{t(time.Unix(v.CreatedUnix, 0).Format("2006-01-02 15:04:05")), t(v.RequestID), t(v.Username), t(v.ModelName), n64(v.PromptTokens.Int64), n64(v.CacheTokens), n64(billed), n64(v.CompletionTokens.Int64), d(charge.InputPrice), d(charge.OutputPrice), d(charge.CachePrice), d(charge.InputAmount), d(charge.OutputAmount), d(charge.CacheAmount), d(charge.Total), n64(v.Quota)})
			rowNo++
		}
