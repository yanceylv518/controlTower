...
type BillingTokenStore interface {
	BillingSummaryStore
	QueryBillingTokenRows(context.Context, string, int64, int64) ([]billing.TokenDailyRow, error)
}
...
func (h BillingTokenHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.Store.QueryBillingTokenRows(r.Context(), job.ID, userID, tokenID)
	if err != nil {
	...
	missing := false
	if len(rows) == 0 {
		userRows, queryErr := h.Store.QueryBillingAggregatesForJob(r.Context(), job.ID, []int64{userID})
		if queryErr != nil {
			writeDashboardError(w, 500, "billing_query_failed")
			return
		}
	...
	writeDashboardJSON(w, 200, map[string]any{"items": billing.BuildTokenSummaries(rows, prices, ratios, snapshots), "token_data_missing": missing, "job": job})
}
...
func writeTokenCSV(w http.ResponseWriter, userID, tokenID int64, from, to time.Time, summaries []billing.TokenSummary, details []billing.DetailItem) {
	...
	_ = cw.Write([]string{"令牌ID", "令牌名", "请求数", "普通输入Token", "缓存读取Token", "缓存写入Token", "输出Token", "CT金额", "Quota"})
	for _, v := range summaries {
		_ = cw.Write([]string{strconv.FormatInt(v.TokenID, 10), v.TokenName, strconv.FormatInt(v.RequestCount, 10), strconv.FormatInt(v.PromptTokens, 10), strconv.FormatInt(v.CacheTokens, 10), strconv.FormatInt(v.CacheWriteTokens, 10), strconv.FormatInt(v.CompletionTokens, 10), v.CTAmount, strconv.FormatInt(v.Quota, 10)})
	}
	_ = cw.Write([]string{})
	_ = cw.Write([]string{"令牌日账单"})
	_ = cw.Write([]string{"日期", "模型", "分组", "档位", "请求数", "普通输入Token", "缓存读取Token", "缓存写入Token", "输出Token", "金额"})
	for _, v := range details {
		_ = cw.Write([]string{v.Day, v.ModelName, v.GroupName, strconv.FormatInt(v.TierFrom, 10), strconv.FormatInt(v.RequestCount, 10), strconv.FormatInt(v.PromptTokens, 10), strconv.FormatInt(v.CacheTokens, 10), strconv.FormatInt(v.CacheWriteTokens, 10), strconv.FormatInt(v.CompletionTokens, 10), v.Amount})
	}
	cw.Flush()
}
