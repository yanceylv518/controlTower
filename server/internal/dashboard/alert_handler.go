...
func appendInstanceOfflineAlerts(items []AlertItem, instances []storage.Instance, agents []storage.Agent, now time.Time, offlineSeconds int) []AlertItem {
	...
	for _, instance := range instances {
		...
		items = append(items, AlertItem{ID: alertID(instance.ID, "instance_offline", instance.ID), InstanceID: instance.ID, RuleKey: "instance_offline", Severity: "critical", Status: "firing", Title: siteID + " 节点离线", Summary: fmt.Sprintf("站点 %s 节点 %s 最后心跳于 %d 分钟前", siteID, nodeName, minutes), SeenAt: agent.LastSeenAt, FirstSeenAt: agent.LastSeenAt, LastSeenAt: agent.LastSeenAt})
	}
	return items
}
