...
func (s Store) ListChannelBaseValues(instanceID, model string) ([]tuning.ChannelBaseValue, error) {
	...
	rows, err := s.db.QueryContext(context.Background(), `SELECT b.instance_id,b.channel_id,c.channel_name,COALESCE(c.group_name,''),b.model_name,b.base_weight,b.base_priority,b.max_rpm,b.max_tpm,c.weight,COALESCE(c.priority,0),c.captured_at,COALESCE(c.models_text,''),b.updated_at,b.updated_by
...
WHERE b.instance_id=? AND b.model_name<>'' AND (LOWER(c.status) IN ('enabled','enable','active','normal','1') OR EXISTS (SELECT 1 FROM tuning_continuous_states ts WHERE ts.instance_id=b.instance_id AND ts.channel_id=b.channel_id AND ts.circuit_disabled=1))`+filter+` ORDER BY b.model_name,c.channel_name`, args...)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&v.InstanceID, &v.ChannelID, &v.ChannelName, &v.GroupName, &v.ModelName, &v.BaseWeight, &v.BasePriority, &v.MaxRPM, &v.MaxTPM, &v.CurrentWeight, &v.CurrentPriority, &v.SnapshotAt, &modelsText, &v.UpdatedAt, &v.UpdatedBy); err != nil {
			return nil, err
		}
...
func (s Store) SaveChannelBaseValuesWithAudit(instanceID, actor string, values []tuning.ChannelBaseValue, now time.Time, auditContext storage.OperationAudit) error {
	...
	for _, v := range values {
		...
		var beforeMaxRPMValue, beforeMaxTPMValue any
		if beforeMaxRPM.Valid {
			beforeMaxRPMValue = beforeMaxRPM.Int64
		}
...
SUM(COALESCE(output_direct_tokens,0)),SUM(COALESCE(output_direct_seconds,0)),` + strings.Join(histogramSums, ",") + `,` + tuningSpeedSumsSQL() + `,SUM(COALESCE(ttft_count,0)),SUM(COALESCE(output_direct_samples,0)),SUM(COALESCE(output_retry_samples,0)),SUM(COALESCE(output_unknown_samples,0)),COALESCE(MAX(output_speed_samples IS NOT NULL),0)
FROM metric_1m
...
func (s Store) QueryMetrics(id string, start, end time.Time) ([]tuning.ChannelMetric, error) {
	...
	for rows.Next() {
		...
		dest = append(dest, &m.SpeedRetries, &m.SpeedUnknown, &totalTTFT, &m.OTPSSamples, &m.OTPSRetries, &m.OTPSUnknown, &m.OTPSStatsVersion)
...
func (s Store) PutContinuousState(v tuning.ContinuousState) error {
	started := time.Now()
	capacityJSON, marshalErr := json.Marshal(v.Capacity)
	if marshalErr != nil {
		return marshalErr
	}
...
func (s Store) CreateContinuousWeightChangeWithAudit(v tuning.Recommendation, actor string, now time.Time, auditContext storage.OperationAudit) (string, error) {
	...
	defer tx.Rollback()
	if err := checkContinuousCapacity(tx, v, time.Now().UTC(), ""); err != nil {
		return "", err
	}
	...
	if err = linkCapacityCommand(tx, v, commandID); err != nil {
		return "", err
	}
	if v.ProposedChannelStatus != nil {
...
func (s Store) HasExpiredAutoCommands(id string, since time.Time) (bool, error) {
	var found int
	err := s.db.QueryRowContext(context.Background(), `SELECT EXISTS(SELECT 1 FROM channel_commands WHERE instance_id IN (SELECT id FROM instances WHERE CASE WHEN site_id='' THEN id ELSE site_id END=?) AND created_by='system:auto' AND status='expired' AND error_summary<>'capacity decision or mode changed' AND updated_at>=?)`, id, since).Scan(&found)
	return found == 1, err
}
