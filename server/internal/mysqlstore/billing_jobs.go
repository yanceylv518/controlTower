...
func (s Store) QueryBillingAnomalies(ctx context.Context, site string, userID, channelID int64, from, to, cursorTime time.Time, cursorID int64, limit int) ([]billing.AnomalyOrder, error) {
	q := `SELECT instance_id,source_log_id,job_id,created_at,request_id,upstream_request_id,user_id,username,channel_id,channel_name,model_name,group_name,prompt_tokens,completion_tokens,cache_tokens,cache_write_tokens,cache_write_5m_tokens,cache_write_1h_tokens,quota,max_context_tokens,input_price,output_price,cache_price,cache_write_price,input_amount,output_amount,cache_amount,cache_write_amount,reference_amount,reasons,detected_at FROM billing_anomaly_orders WHERE instance_id=? AND created_at>=? AND created_at<? AND (created_at>? OR (created_at=? AND source_log_id>?))`
