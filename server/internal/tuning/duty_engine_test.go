...
func TestDutyDemoteSustainedSevereAndTiedActive(t *testing.T) {
	...
	f := &fakeStore{policy: p, channels: channels, metrics: []ChannelMetric{{1, 100, 20, 1}, {2, 100, 20, 1}}}
	e := NewEngine(f)
	now := time.Now().UTC()
	e.Tick(now)
...
func TestDutyLatencyMixedAndBackupBranches(t *testing.T) {
	...
	f := &fakeStore{
		...
		metrics:  []ChannelMetric{{1, 100, 0, 11}},
	}
	e := NewEngine(f)
	e.Tick(now)
	e.Tick(now.Add(time.Minute))
	...
	single := &fakeStore{policy: p, channels: []Channel{dutyChannel(1, 100, "m")}, metrics: []ChannelMetric{{1, 100, 50, 1}}}
	...
	exhausted := &fakeStore{
		...
		metrics:  []ChannelMetric{{1, 100, 50, 1}},
...
func TestDutyBaselineInsufficientCooldownAndDailyLimit(t *testing.T) {
	...
	insufficient := &fakeStore{
		...
		channels: channels, metrics: []ChannelMetric{{1, 100, 0, 50}},
	}
