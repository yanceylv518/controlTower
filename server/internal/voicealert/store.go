...
func (s Store) Snapshot(ctx context.Context, t Target, now time.Time) ([]int64, time.Time, error) {
	...
	for _, src := range sources {
		marks, e := s.DB.QueryContext(ctx, `SELECT bucket_time FROM user_rate_seconds WHERE instance_id=? AND user_id=0 AND bucket_time>=? AND bucket_time<=? ORDER BY bucket_time`, src.id, start.Add(-60*time.Second), src.end)
		if e != nil {
			return nil, end, e
		}
