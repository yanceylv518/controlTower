...
- 新增站点级渠道目录接口，读取每个站点最新的 `channel_current`，不受调权基础值接口的启用状态、单模型限制影响。
- 新增逐渠道分组编辑入口，只允许选择当前站点 New API 渠道快照中已存在的分组、复用已有组合，以及清空全部分组；不允许手工创建分组。
- 使用 New API 现有的逗号字符串合同，例如 `default,vip,fast`；服务端统一去首尾空格、去重，并拒绝逗号空项、控制字符和超过 128 个 Unicode 字符的组合。
- 非空组合中的每个分组都必须已经出现在当前站点的渠道快照中；未知分组返回 `400 group_not_found`，不会创建命令或写入 New API。
...
空字符串表示清空分组。非空组合只能使用当前站点渠道快照已有的分组名称。直连成功返回 HTTP 200，Agent 队列返回 HTTP 202；格式或未知分组返回 400，站点中找不到渠道返回 404。
